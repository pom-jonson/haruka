...
const UNEVEN_OPTIONS = [
  {id: 0, label: "選択", value: "" },
  {id: 1, label: "起床時", value: "起床時" },
  {id: 2,  label: "朝食後", value: "朝食後" },
  {id: 3, label: "昼食前", value: "昼食前" },
  {id: 4, label: "昼食後", value: "昼食後" },
  {id: 5, label: "夕食前", value: "夕食前" },
  {id: 6, label: "夕食後", value: "夕食後" },
  {id: 7, label: "寝る前", value: "寝る前" }
];
...
const Key = ({ children, onClick, setHoverStyle }) => <div onClick={onClick} onMouseOver={setHoverStyle}>{children}</div>;
Key.propTypes = {
...
class Calc extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState(
      ...
      () => {
        var partUsageData = [];
        this.state.usageData.external.all.map(usage => {
          partUsageData.push({
            label: usage.name,
            value: usage.name,
            usage: usage.code
        ...
        this.state.usageData.injection.all.map(usage => {
          partUsageData.push({
            label: usage.name,
            value: usage.name,
            usage: usage.code
  ...
  setHoverStyle=(value)=>{
    this.hover_count++;
    if(this.hover_count == 1){
      return;
    }
    if(value){
      let css = '.wrap-area div:hover{ background-color: '+(colors.background)+' }';
      ...
      document.getElementById('calc_dlg').appendChild(style); 
      this.hover_count = 0;
    }
  }
  ...
  calcConfirm = (keypressed = false, item) => {
    ...
    if ( this.state.tabIndex != 2 && ((parseFloat(this.state.formula)*10000/10000) <= 0 || this.state.formula === "")) {
    ...
    if (this.state.tabIndex == 2) {
      ...
      formula = this.convertAmount(formula);
      window.localStorage.removeItem(CACHE_LOCALNAMES.CALC);
      ...
      if (this.state.tabIndex === 3) {
        if (this.state.partUsageItem === undefined) return;
        if (
          this.state.partUsageItem.usageName === undefined ||
          this.state.partUsageItem.usageName === ""
        ) {
          window.sessionStorage.setItem("alert_messages", "用法を選択してください。");
          return;
        }
        if (
          this.state.partUsageItem.usageComment === undefined ||
          this.state.partUsageItem.usageComment === ""
        ) {
          window.sessionStorage.setItem("alert_messages", "部位/補足を選択してください。");
          return;
        }
        if (this.state.partUsageItem.usageName.includes("ＸＸ")) {
          if (
            this.state.partUsageItem.usage_optional_num === undefined ||
            this.state.partUsageItem.usage_optional_num === null ||
            this.state.partUsageItem.usage_optional_num <= 0
          ) {
            window.sessionStorage.setItem("alert_messages", "1回数量を入力してください。");
            return;
          }
          if (
            this.state.partUsageItem.poultice_times_one_day === undefined ||
            this.state.partUsageItem.poultice_times_one_day === null ||
            this.state.partUsageItem.poultice_times_one_day <= 0
          ) {
            window.sessionStorage.setItem("alert_messages", "1日回数を入力してください。");
            return;
          }
        }

        if (
          this.state.partUsageItem.days === undefined ||
          this.state.partUsageItem.days === null ||
          this.state.partUsageItem.days <= 0
        ) {
            window.sessionStorage.setItem("alert_messages", "日数を入力してください。");
          return;
        }
      }
  ...
  /** 2020-08-30
    数量として送信する項目は、小数点以下4桁以内・整数部は最大8桁、「.」含み合計10桁までなので、合計値はこれに合わせて四捨五入。
    ・整数部が8桁あるの場合、小数点以下は1桁まで
    ・整数部7桁あるの場合、小数点以下が2桁まで
    ・整数部6桁あるの場合、小数点以下は3桁まで
    ・整数部が5桁以下の場合、小数点以下は4桁まで。
   */
  convertAmount = (amount) => {
    let amount_val = amount.toString().split(".");
    let return_val = amount;
    if (amount_val[1] == undefined) return parseFloat(amount);
    if (amount_val[0].toString().length == 8 && amount_val[1].toString().length > 1) {
      return_val = parseFloat(amount).toFixed(1);
    } else if (amount_val[0].toString().length == 7 && amount_val[1].toString().length > 2) {
      return_val = parseFloat(amount).toFixed(2);
    } else if (amount_val[0].toString().length == 6 && amount_val[1].toString().length > 3) {
      return_val = parseFloat(amount).toFixed(3);
    } else if (amount_val[0].toString().length <= 5 && amount_val[1].toString().length > 4) {
      return_val = parseFloat(amount).toFixed(4);
    }
    return parseFloat(return_val);
  };
  ...
  onKeyPressed(e) {
    ...
    if (e.currentTarget == e.target) {
      ...
      if (e.keyCode > 47 && e.keyCode < 58 && tabIndex !== 3) {
        ...
        if (this.props.units.length <= 0) return;
  ...
  onChangeUsageName = (index, e) => {
    let unevenValues = this.state.unevenValues;
    if (this.state.unevenSelectedIndex >= 0) {
      unevenValues[index].label = e.target.value;
    }
    if (index === this.state.unevenValues.length - 1) {
      unevenValues.push({
        label: UNEVEN_OPTIONS[0].label,
        value: "0.000"
      });
  ...
  amountBlur = (index) => {
    let unevenValues = this.state.unevenValues;
    unevenValues[index].value = isNaN(parseFloat(this.state.formula)) ? "0.000" : parseFloat(this.state.formula).toString();
    this.setState({
      formula: unevenValues[index].value,
      unevenValues
    });
  ...
  render() {
    ...
    if(units !== undefined) {
      units.map((item)=>{
        if(item.main_unit_flag == 1) main_unit = item.name;
      });
    }
    return (
      <Modal
          ...
          >
            {units.length > 0 && (
              <CalcNav
                ...
                id={tabIndex}
                showedPresData={this.props.showedPresData}
              />
            )}
            {this.props.daysLabel !== "" && (
              <div className="btn-box align-center">
                <p className="align-center" style={{marginBottom:0}}>
            ...
            {(tabIndex == 1 || tabIndex == 2) && (
              <>
                {tabIndex === 2 && units.length > 0 && (
                  <UnitBoxTop>
                    <p className="label" style={{marginBottom:0}}>単位選択</p>
                    <ol>
                      {units.map((unit, index) => (
                        <li
                          ...
                          title={unit.main_unit_flag != 1 ? `1${unit.name} = ${unit.multiplier}${main_unit}`: ""}
                        >
                          {unit.name}
                        </li>
                      ))}
                ...
                <UnevenContainer className={tabIndex === 2 ? "move-top" : ""}>
                          ...
                          return (
                            <div className="uneven" key={index}>
                              <SelectorWithLabel
                                ...
                                getSelect={this.onChangeUsageName.bind(this, index)}
                                departmentEditCode={UNEVEN_OPTIONS.findIndex(
                  ...
                  <Wrap className={'wrap-area'}>
                    <Display
                      formula={this.state.formula}
                      unit={this.state.unit}
                      class_name={tabIndex === 1 ? "" : "hide"}
                    />
                    <p className={tabIndex === 1 ? "width-80" : "hide width-70"} style={{marginBottom:0}}>
                    ...
                    {KEYS.map(([key, value]) => (
                      <Key
                        ...
                        setHoverStyle={() => this.setHoverStyle(value)}
                      >
                        {key}
                      </Key>
                    ))}
