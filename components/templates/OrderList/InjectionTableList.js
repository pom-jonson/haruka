...
const ContextMenu = ({ visible, x,  y,  parent,  row_index, authenticate_done, input_result}) => {
  if (visible) {
    return (
      <ContextMenuUl>
        <ul className="context-menu" style={{ left: `${x}px`, top: `${y}px` }}>
          ...
          {authenticate_done == 1 && (
            <li><div onClick={() => parent.contextMenuAction(row_index,"authenticate_done")}>3点認証実施</div></li>
          )}
          {input_result == 1 && (
            <li><div onClick={() => parent.contextMenuAction(row_index,"input_result")}>結果入力</div></li>
          )}
        </ul>
      </ContextMenuUl>
    );
...
class InjectionTableList extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      selected_index:'',
      alert_messages:"",
      print_numbers:props.print_numbers,
      isOpenPatientCertification:false,
    };
    this.use_varcode_print = 0;
    this.authenticate_done_menu_view = 1;
    this.input_result_menu_view = 1;
    let initState = JSON.parse(sessApi.getValue(CACHE_SESSIONNAMES.INIT_STATUS));
    if(props.type == 'hospital_order_list' && initState !== undefined && initState != null && initState.conf_data !== undefined){
      if(initState.conf_data.hospital_injection_receive_use_varcode_print !== undefined){
        this.use_varcode_print = initState.conf_data.hospital_injection_receive_use_varcode_print;
      }
      if(initState.conf_data.hospital_injection_receive_authenticate_done_menu_view !== undefined){
        this.authenticate_done_menu_view = initState.conf_data.hospital_injection_receive_authenticate_done_menu_view;
      }
      if(initState.conf_data.hospital_injection_receive_input_result_menu_view !== undefined){
        this.input_result_menu_view= initState.conf_data.hospital_injection_receive_input_result_menu_view;
      }
    }
  }
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    this.setState({
      ...
      print_numbers:nextProps.print_numbers
    },()=>{
  ...
  handleClick = (e, index) => {
    if (e.type === "contextmenu") {
      ...
      this.setState({
        ...
        row_index: index,
        selected_index:index,
        authenticate_done:this.authenticate_done_menu_view,
        input_result:this.input_result_menu_view,
      }, ()=>{
  ...
  contextMenuAction = (index, type) => {
    ...
    if (type === "authenticate_done" || type === "input_result"){
      this.setState({
        isOpenPatientCertification:true,
        authenticate_data:this.state.table_list[index],
      });
    }
  };
  closeModal = (act) => {
  ...
  getModalData = (_modal_data) => {
    ...
    ret.input_staff_name = _modal_data.prescription.input_staff_name != undefined && _modal_data.prescription.input_staff_name != null && _modal_data.prescription.input_staff_name != "" ? _modal_data.prescription.input_staff_name : _modal_data.prescription.proxy_input_staff_name;
  ...
  setPrintNumber =(name, number)=>{
    if(name == "print_number"){
      ...
      this.props.setPrintNumber(print_numbers);
    }
  };
    ...
    return (
      <TableHeader>
        <div className={'table-area'}>
          <table className="table-scroll table table-bordered" id="code-table">
            <thead className={'thead-area'}>
              <tr>
                <th style={{width:"8rem"}}>予定日付</th>
                <th style={{width:"10rem"}}>患者ID</th>
                <th style={{width:"15rem"}}>氏名</th>
                <th style={{width:"7rem"}}>実施状態</th>                
                <th>指示医師</th>
                <th style={{width:"14rem"}}>指示日時</th>              
                <th style={{width:"14rem"}}>実施日時</th>
              </tr>
            </thead>
                    ...
                    {this.state.table_list != undefined && this.state.table_list != null && this.state.table_list.length > 0 && this.state.table_list.map((item, index) => {
                      return (
                        <>
                          ...
                          >
                            <td style={{width:"8rem"}}>
                              {item.prescription.schedule_date != null ? this.getConvertDateTime(item.prescription.schedule_date, "type_2"):""}
                            </td>
                            <td style={{width:"10rem",textAlign: "right"}}>
                            ...
                            <td style={{width:"7rem"}}>
                              {item.prescription.done_order == 1 ? "実施済み" : (item.prescription.done_order == 2 ? "受付済み" : "未実施")}
                            </td>
                            <td>
        ...
        <ContextMenu
          ...
          authenticate_done={this.state.authenticate_done}
          input_result={this.state.input_result}
        />
        ...
        {this.state.isOpenPatientCertification && (
          <PatientCertification
            closeModal={this.closeModal}
            authenticate_data={this.state.authenticate_data}
          />
        )}
      </TableHeader>
    );
...
InjectionTableList.contextType = Context;
InjectionTableList.propTypes = {
  ...
  print_numbers:PropTypes.string,
  setPrintNumber:PropTypes.func,
};

export default InjectionTableList;
