...
class EndoscopeList extends Component {
  constructor(props) {
    ...
    var auto_reload_data = getAutoReloadInfo('endscope_accept');
    ...
    let status_row_color = getStatusRowColor('endscope_accept');
  ...
  async componentDidMount() {
    localApi.setValue("system_next_page", "/endoscope/order_list");
    localApi.setValue("system_before_page", "/endoscope/order_list");
      ...
      .then((res) => {
        if(res){
          ...
          if (type === 'init') {
            ...
            if (Math.ceil(res.length/this.state.display_number) < current_page){
              current_page = Math.ceil(res.length/this.state.display_number);
            }
          }
  ...
  printData = () => {
    if (this.state.inspection_table_list === undefined || this.state.inspection_table_list.length == 0 || !this.state.is_loaded)  return;
    this.setState({
      confirm_message:"",
    ...
    axios({
      ...
      if(window.navigator.msSaveOrOpenBlob) {
        //IE11 & Edge
        window.navigator.msSaveOrOpenBlob(blob, '内視鏡受付.pdf');
      }
      else{
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', '内視鏡受付.pdf'); //or any other extension
    ...
    return (
      <PatientsWrapper>
        <div className="title-area flex" style={{borderBottomColor:this.title_border_bottom_color}}>
          <div className={'title'} style={{color:this.title_color, borderLeftColor:this.title_border_left_color}}>内視鏡受付</div>
        ...
        <Flex>
          ...
          <div className={'d-flex'} style={{marginTop:"5px"}}>
            ...
            <div className = 'reload-check d-flex'>
              <Button type="common" className={'mr-3 ' + (button_disable ? 'disable-btn' : '')} isDisabled={button_disable} onClick={this.printData}>一覧印刷</Button>
              <Checkbox
                label={'自動更新'}
