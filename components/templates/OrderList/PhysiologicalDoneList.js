...
class PhysiologicalList extends Component {
  constructor(props) {
    ...
    var auto_reload_data = getAutoReloadInfo('physiological_done');
    ...
    let status_row_color = getStatusRowColor('physiological_done');
  ...
  async componentDidMount() {
    localApi.setValue("system_next_page", "/physiological/done_list");
    localApi.setValue("system_before_page", "/physiological/done_list");
    ...
    let path = "/app/api/v2/master/inspection/get_inspection_master";
    let inspection_master = [{id: 0, value: "全て"}];
    await apiClient._post(path).then(res=>{
      if (res) {
        res.map(item=>{
          if(item.end_until_continue_type == 0){
            inspection_master.push({id:item.inspection_id, value:item.name});
          }
        });
        this.setState({inspection_master});
      }
    });
  ...
  printData = () => {
    ...
    axios({
      ...
      if(window.navigator.msSaveOrOpenBlob) {
        //IE11 & Edge
        window.navigator.msSaveOrOpenBlob(blob, '生理検査実施.pdf');
      }
      else{
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', '生理検査実施.pdf'); //or any other extension
    ...
    return (
      <PatientsWrapper>
        <div className="title-area flex" style={{borderBottomColor:this.title_border_bottom_color}}>
          <div className={'title'} style={{color:this.title_color, borderLeftColor:this.title_border_left_color}}>生理検査実施</div>
