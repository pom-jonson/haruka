...
import {formatDateLine,formatJapanDate,formatJapanDateSlash, formatTimeIE,getNextDayByJapanFormat,getPrevDayByJapanFormat} from "~/helpers/date";
...
class EndoscopeList extends Component {
  constructor(props) {
    ...
    let status_row_color = getStatusRowColor('endscope_done');
    ...
    this.sort_data = this.sort_data.implement;
  ...
  async componentDidMount() {
    localApi.setValue("system_next_page", "/endoscope/done_list");
    localApi.setValue("system_before_page", "/endoscope/done_list");
    ...
    await apiClient._post(
      ...
      .then((res) => {
        if(res){
          ...
          this.setState({ inspection_table_list:res, is_loaded:true, current_page, pageOfItems :[]}, () => {
  ...
  printData = () => {
    if (this.state.inspection_table_list === undefined || this.state.inspection_table_list === null || this.state.inspection_table_list.length == 0 || !this.state.is_loaded)  return;
    this.setState({complete_message:"印刷中"});
    let path = "/app/api/v2/master/inspection/print_list";
    let print_data = {params:this.state, type:"endoscope"};
    axios({
      url: path,
      method: 'POST',
      data:{print_data},
      responseType: 'blob', // important
        ...
        if(window.navigator.msSaveOrOpenBlob) {
          //IE11 & Edge
          window.navigator.msSaveOrOpenBlob(blob, '内視鏡実施.pdf');
        }
        else{
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', '内視鏡実施.pdf'); //or any other extension
  ...
  render() {
    let list_names = ["内視鏡受付", "内視鏡実施"];
    var list_urls = ["/endoscope/order_list", "/endoscope/done_list"];
    const menu_list_ids = ["444", "445"];
    let curUserInfo = JSON.parse(sessApi.getValue('haruka'));
    let arr_menu_permission = curUserInfo.menu_auth;
    let button_disable = false;
    if (this.state.inspection_table_list === undefined || this.state.inspection_table_list === null || this.state.inspection_table_list.length == 0 || !this.state.is_loaded) button_disable = true;
    return (
      <PatientsWrapper>
        <div className="title-area flex" style={{borderBottomColor:this.title_border_bottom_color}}>
          <div className={'title'} style={{color:this.title_color, borderLeftColor:this.title_border_left_color}}>内視鏡実施</div>
        ...
        <div className={'table-area'}>
          <table className="table-scroll table table-bordered" id="code-table">
            ...
            <tr>
              ...
              <th style={{width:"17rem"}}>カナ</th>
              {/*<th style={{width:"10rem"}}>オーダーの種類</th>*/}
              <th style={{width:"12rem"}}>指示医師</th>
              <th style={{width:"6rem"}}>実施状態</th>
              <th>実施日時
                      ...
                      return (
                        <>
                          ...
                          >
                            ...
                            <td style={{width:"12rem"}}>{item.order_data !== undefined && item.order_data.order_data.doctor_name !== undefined && item.order_data.order_data.doctor_name}</td>
                            <td style={{width:"6rem"}}>{item.state == 0 ? "未受付" : (item.state == 1 ? "受付済み" : "実施")}</td>
                            <td>
                              <div>{item.completed_at != null ? (formatJapanDateSlash(item.completed_at) + formatTimeIE(item.completed_at.split('-').join('/'))) : ""}</div>
...
EndoscopeList.contextType = Context;
EndoscopeList.propTypes = {
  history: PropTypes.object,
}

export default EndoscopeList;
