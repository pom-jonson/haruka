...
import EditDailysisPrescriptionSchedulModal from "./modals/EditDailysisPrescriptionSchedulModal";
            ...
            <TooltipTitleMenuUl>
                ...
                {title.length > 0 && title.map(manage_item=>{
                    return (
                      <>
                        <div>
                          {manage_item.medicine_name != undefined ? manage_item.medicine_name + ": " : ""} 
                          {manage_item.amount != undefined ? manage_item.amount : ""} 
                          {manage_item.unit != undefined ? manage_item.unit + " ": ""}
                          {manage_item.timing_name != undefined ? manage_item.timing_name : ""}</div>
                     </>
                    )
                  })}
                </ul>
    ...
    return (
      <ContextMenuUl>
        <ul className="context-menu" style={{ left: `${x}px`, top: `${y}px` }}>
          ...
          <li><div onClick={() => parent.goOtherPattern("/dial/pattern/dialPrescript", item.system_patient_id)}>透析中処方パターンへ</div></li> 
...
const ContextMenu_patient = ({visible, x, y,parent,  system_patient_id}) => {
  if (visible) {
    return (
      <ContextMenuUl>
        <ul className="context-menu" style={{ left: `${x}px`, top: `${y}px` }}>
          <li><div onClick={() => parent.goOtherPattern("/dial/pattern/dialPrescript", system_patient_id)}>透析中処方パターンへ</div></li> 
...
const ContextMenu_other = ({visible, x, y,parent, day, patient_id}) => {
  if (visible) {
    return (
      <ContextMenuUl>
        <ul className="context-menu" style={{ left: `${x}px`, top: `${y}px` }}>
          <li> <div onClick={() => parent.showAddModal(day, patient_id)}>透析中処方スケジュール追加</div></li>
          <li><div onClick={() => parent.goOtherPattern("/dial/pattern/dialPrescript", patient_id)}>透析中処方パターンへ</div></li>
...
class DialysisPrescription extends Component {
    constructor(props) {            
      super(props);
    ...
    handleOk = (schedule_date) => {
      this.props.getSearchResult(new Date(schedule_date), 2);
      this.closeModal();
    };
...
DialysisPrescription.contextType = Context;

DialysisPrescription.propTypes = {
...
export default withRouter(DialysisPrescription)