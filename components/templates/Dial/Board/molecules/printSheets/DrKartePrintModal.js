...
class DrKartePrintPreviewModal extends Component {
  ...
  get_title_pdf = () => {
    let server_time = formatDateLine(this.props.end_date).split("-").join("");
    // 本日
    if (this.props.select_date_type == 2) {
      server_time = formatDateLine(this.props.start_date).split("-").join("") + "-" + formatDateLine(this.props.end_date).split("-").join("");
    }
...
DrKartePrintPreviewModal.propTypes = {
  ...
  select_date_type :PropTypes.number,
