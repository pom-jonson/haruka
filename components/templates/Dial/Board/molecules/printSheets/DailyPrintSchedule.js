...
class DailyPrintSchedule extends Component {
  constructor(props) {
    super(props);
    this.state = {
      numPages: null,
  ...
  async componentDidMount() {
    // download pdf
    var url;
    url = '/app/api/v2/dial/generatepdf/schedule_table';
  ...
  get_title_pdf=()=>{
    var title = 'スケジュール表_';
    title += formatDateLine(this.props.schedule_date).split('-').join('');
    return title + ".pdf";
  }
  ...
  render() {
    ...
    return  (
      <Modal show={true} onHide={this.onHide} id="add_contact_dlg_print"  className="master-modal medical-info-doc-preview-modal blood-record-modal">
        <Modal.Header>
          <Modal.Title>スケジュール表</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
...
DailyPrintSchedule.contextType = Context;

DailyPrintSchedule.propTypes = {
...
export default DailyPrintSchedule;
