        ...
        <ul className="context-menu main-context-menu" style={{ left: `${x}px`, top: `${y}px` }}>
          {medi_index != undefined && (
            <>
              ...
              <li>
                <div onClick={() => parent.contextMenuAction(rp_index,medi_index,"amount_change")} onMouseOver = {e => parent.outMainHover(e)}>
                  数量変更
                </div>
              </li>
...
class Prescription extends Component {
  ...
  getCheckedLast =async (name, value) => {
    if (name === "last") {
      if (value == 0) {
        ...
        let tmp = [];
        let today = formatDateLine(new Date(server_time));
        tmp = this.getPatternListByDateCondition(this.state.prescription_pattern,today,"time_limit_from","time_limit_to");
        this.setState({
  ...
  clear = () => {
    if (!this.state.change_flag) return;
  ...
  selectPatient =async(patient) => {
    ...
    await this.getPrescriptionPatternInfo(patient.system_patient_id);
    this.setDoctors();
  };
  ...
  editPattern = () => {
    this.initRedBorder();
    this.setChangeFlag(0);
    let pattern = this.state.prescription_pattern !== undefined && this.state.selected_pattern_data;
    ...
    let _state = {
      selected_number:pattern.number,
      number:pattern.prescription_number !== undefined && pattern.prescription_number,
      final_week_days: weekday,
      checkalldays: final_week_days,
      time_limit_from: new Date(pattern.time_limit_from),
      time_limit_to: pattern.time_limit_to != null ? new Date(pattern.time_limit_to) : "",
  ...
  goOtherPattern = (url) => {
    ...
    if (url == "/dial/schedule/Schedule")
      sessApi.setObjectValue("dial_schedule_table", "open_tab", "medicine");
  ...
  handleClick = (e, rp_index, medi_index, comment_index) => {
    if (e.type === "contextmenu") {
      ...
      this.setState({
        ...
        if (clientX + main_menu.offsetWidth > window.innerWidth - 190){
          this.setState({
            contextMenu: {
              visible: true,
              x: clientX - main_menu.offsetWidth,
  ...
  setPatternHover = (e,  rp_index, comment_index, pattern_number) => {
    ...
    this.setState({
      ...
      if(clientY + word_menu.offsetHeight > window.innerHeight){
        if (this.state.pattern_menu_reserved_flag){
          this.setState({
            hoverWordMenu: {
              visible: true,
  ...
  setMainHover = (e,  rp_index,  comment_index) => {    
    ...
    this.setState({
      ...
      if (hover_pattern_x + pattern_menu.offsetWidth + 230 > window.innerWidth - 190){     
        if (hover_pattern_y + pattern_menu.offsetHeight > window.innerHeight){
          this.setState({
            hoverPatternMenu: {
              visible: true,
  ...
  contextMenuAction = (rp_index, medi_index, type, comment_index) => {
    if (type === "amount_change") {
      ...
      if(free_comment.length == 0 || comment_index === undefined) {
        ...
        free_comment.splice(comment_index + 1, 0, "");
      }
      data_json_item.free_comment = free_comment;
      data_json[rp_index] = data_json_item;
      this.setState({
  ...
  inputUsage = () => {
    ...
    if (
      this.state.selected_rp == undefined ||
      this.state.selected_rp == null ||
  ...
  setChangeFlag = (change_flag) => {
    ...
    if (change_flag) {
      sessApi.setObjectValue("dial_change_flag", "prescription", 1);
    } else {
  ...
  render() {
    ...
    if (this.state.prescription_pattern.length) {
      prescription_pattern = this.state.prescription_pattern.map((item) => {
          let weekday = "";
          for (let i = 0; i < 7; i++) {
          ...
          return (
            <>
              ...
              >
                <div className={"pattern-number bl-1p bt-1p bb-1p"}>
    ...
    return (
      <>
        ...
        <Card>
          <div className={"flex"}>
            <div className="title">処方パターン</div>
            <div className={"other-pattern"}>
              ...
              <Button className="disable-button">処方</Button>
              <Button
                onClick={this.goOtherPattern.bind(
                  this,
              ...
              <div className="main-area">
                ...
                <div className="dial-oper">
                  ...
                  <div className="d-flex">
                    ...
                    <div style={{width:"25.25rem", marginLeft:"auto"}}>
