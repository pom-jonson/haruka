...
class OtherFacilitiesModal extends Component {
  constructor(props) {
    super(props);
  ...
  async saveOtherFacilities() {
    let path = "/app/api/v2/dial/master/other_facilities_register";
    const post_data = {
      params: this.state,
    };
  ...
  handleOk = () => {
    ...
    if (this.props.modal_data !== null) {
      this.setState({
        ...
        confirm_message: "他施設マスタ情報を変更しますか?",
      });
    } else {
      this.setState({
        isUpdateConfirmModal: true,
        confirm_message: "他施設マスタ情報を登録しますか?",
      });
    }
  };
  ...
  //   let validate_data = master_validate.dial_other_facilities_master;
  //   if (
  ...
  render() {
    ...
    return (
      ...
      >
        <Modal.Header>
          <Modal.Title>
            他施設マスタ{this.props.modal_data != null ? "編集" : "登録"}
            {category}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
OtherFacilitiesModal.contextType = Context;

OtherFacilitiesModal.propTypes = {
...
export default OtherFacilitiesModal;
