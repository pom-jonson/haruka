...
class BasicDataPreviewModalDom extends Component {
  constructor(props) {
    super(props);
    this.state = {
      numPages: null,
  ...
  async componentDidMount() {
    axios({
      url: '/app/api/v2/dial/print/basic_data',
      method: 'POST',
      data:{
  ...
  get_title_pdf = () => {
    let pdf_file_name = "基礎データ入力" + "_" + this.props.schedule_date.split("-").join("") + ".pdf";
  ...
  render() {
    ...
    return  (
      <Modal show={true} onHide={this.onHide} id="add_contact_dlg"  className="master-modal medical-info-doc-preview-modal">
        <Modal.Header>
          <Modal.Title>基礎データ入力</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
...
BasicDataPreviewModalDom.contextType = Context;

BasicDataPreviewModalDom.propTypes = {
  closeModal: PropTypes.func,
  schedule_date : PropTypes.string,
  pres_data: PropTypes.object,
};

export default BasicDataPreviewModalDom;
