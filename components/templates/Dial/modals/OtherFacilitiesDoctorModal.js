...
class OtherFacilitiesDoctorModal extends Component {
  constructor(props) {
    super(props);
    let modal_data = this.props.modal_data;
    let otherFacilitiesNumber = this.props.selectedOtherFacilitiesNumber;
    let otherFacilitiesDepartmentNumber = this.props
      .selectedOtherFacilitiesDepartmentNumber;
    this.state = {
      ...
      other_facilities_department_number:
        modal_data !== null
          ? modal_data.other_facilities_department_number
          : otherFacilitiesDepartmentNumber,
  ...
  async saveOtherFacilitiesDepartment() {
    let path = "/app/api/v2/dial/master/other_facilities_doctor_register";
    const post_data = {
      params: this.state,
    };
  ...
  handleOk = () => {
    ...
    if (this.props.modal_data !== null) {
      this.setState({
        ...
        confirm_message: "担当医情報を変更しますか?",
      });
    } else {
      this.setState({
        isUpdateConfirmModal: true,
        confirm_message: "担当医情報を登録しますか?",
      });
  ...
  //   let validate_data = master_validate.dial_other_facilities_master_doctor;
  //   if (
  ...
  render() {
    ...
    return (
      ...
      >
        <Modal.Header>
          <Modal.Title>担当医編集{category}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
        ...
        <Modal.Footer>            
            ...
            {/*{this.state.modal_data === null && (
              <>
                <div onClick={this.temp_save} className={"custom-modal-btn red-btn"} style={{cursor:"pointer"}}><span>続けて登録</span></div>            
              </>
            )}*/}                    
        </Modal.Footer>
      </Modal>
...
OtherFacilitiesDoctorModal.contextType = Context;

OtherFacilitiesDoctorModal.propTypes = {
  ...
  selectedOtherFacilitiesDepartmentNumber: PropTypes.integer,
};

export default OtherFacilitiesDoctorModal;
