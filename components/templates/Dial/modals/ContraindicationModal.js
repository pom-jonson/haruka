...
const display_depart = ["禁忌", "注意"];

class ContraindicationModal extends Component {
  constructor(props) {
    super(props);
    ...
    if (
      ...
      if (
        modal_data != null &&
        modal_data.date != undefined &&
        modal_data.date != null
      ) {
        type_date = 0;
        date = modal_data.date;
      } else if (
        modal_data != null &&
        modal_data.month != undefined &&
        modal_data.month != null
      ) {
        date =
          modal_data.year.toString() +
          "-" +
          ("00" + modal_data.month.toString()).slice(-2);
        type_date = 1;
      } else {
    ...
    this.state = {
      id: modal_data != null && modal_data != undefined ? modal_data.id : null,
      date:
        date != undefined && date != null && date != "" ? new Date(date) : "",
      note:
      ...
      type_date: type_date,
      type_contraindicant:
        modal_data !== null && modal_data.type_contraindicant !== undefined
          ? modal_data.type_contraindicant
          : "禁忌",
      contraindicant_code:
        modal_data !== null && modal_data.contraindicant_code !== undefined
          ? modal_data.contraindicant_code
          : "",
      number: modal_data != null && modal_data != undefined ? modal_data.id : 0,
      data_list,
  ...
  changeBackground = () => {
    medicalInformationValidate("contraindication", this.state, "background");
  }
  ...
  getSelect = (e) => {
    this.setState({ contraindicant_code: e.target.id });
  };

  selectDepart = (value) => {
    this.setState({ type_contraindicant: value });
  };
  ...
  handleOk = () => {
    let validate_data = medicalInformationValidate("contraindication", this.state);
  ...
  register = () => {
    let postData = {
      ...
      type_contraindicant: this.state.type_contraindicant,
      contraindicant_code: this.state.contraindicant_code,
    };
    if (
  ...
  render() {
    let tooltip = "";
    if (this.state.contraindicant_code == "") {
      tooltip = "名称を選択してください。";
    } else if (this.state.date == null || this.state.date === "") {
      tooltip = "日付を選択してください。";
    } else if (this.state.type_contraindicant === "") {
      tooltip = "区分を選択してください。";
    }
            ...
            <div className="flex">
              <div className="left-area">
                <SelectorWithLabel
                  ...
                  title="名称"
                  id='contraindicant_code_id'
                  getSelect={this.getSelect.bind(this)}
                  departmentEditCode={this.state.contraindicant_code}
                />
                <div className="flex">
                ...
                <div className="display-day">
                  <label>区分</label>
                  <>
                    {display_depart.map((item, key) => {
                      return (
                        <>
                          <RadioButton
                            id={`display_depart_${key}`}
                            value={item}
                            label={item}
                            name="display_depart"
                            getUsage={this.selectDepart.bind(this, item)}
                            checked={this.state.type_contraindicant == item}
                          />
                        </>
                      );
                    })}
...
ContraindicationModal.contextType = Context;

ContraindicationModal.propTypes = {
...
export default ContraindicationModal;
