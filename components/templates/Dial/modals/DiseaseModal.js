...
class DiseaseModal extends Component {
  ...
  changeBackground = () => {
    masterValidate("dial_disease_name_master", this.state, 'background');    
  };
  ...
  checkValidation = () => {
    ...
    removeRedBorder("sort_number_id");
    let error_str_arr = [];
    let validate_data = masterValidate("dial_disease_name_master", this.state);
  ...
  getValue = (key, e) => {
    ...
    else if (key === "category") this.setState({ category: e.target.value });
  ...
  handleOk = () => {
    ...
    // let validate_data = master_validate.dial_disease_name_master;
    // if (
    ...
    if (this.props.modal_data !== null) {
      this.setState({
        ...
        confirm_message: "病名マスタ情報を変更しますか?",
      });
    } else {
      this.setState({
        isUpdateConfirmModal: true,
        confirm_message: "病名マスタ情報を登録しますか?",
      });
    }
  };
  ...
  render() {
    ...
    return (
      ...
      >
        <Modal.Header>
          <Modal.Title>病名マスタ編集</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
            ...
            <div className="disease_classification">
              <label className="checkbox-label">病名区分</label>
              <RadioButton
                id="disease_name_modal"
                value="病名"
                label="病名"
                name="disease_kind_modal"
                getUsage={this.getValue.bind(this, "category")}
...
DiseaseModal.contextType = Context;
