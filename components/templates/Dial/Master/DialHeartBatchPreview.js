...
class DialHeartBatchPreview extends Component {
  constructor(props) {
    super(props);
    this.state = {
      numPages: null,
  ...
  async componentDidMount() {
    let path = "/app/api/v2/dial/heart_batch/print";
    let print_data = {};
  ...
  get_title_pdf = () => {
    ...
    let pdf_file_name = this.props.modal_title + "_" + formatDateLine(this.props.schedule_date).split("-").join("") + ".pdf";
...
DialHeartBatchPreview.contextType = Context;

DialHeartBatchPreview.propTypes = {
...
export default DialHeartBatchPreview;
