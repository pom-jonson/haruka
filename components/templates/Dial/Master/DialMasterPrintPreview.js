...
class DialMasterPrintPreview extends Component {
  ...
  async componentDidMount() {
    let path = "/app/api/v2/dial/master/print";
    let print_data = {};    
    ...
    print_data.search_class = this.props.search_class;
    print_data.sort_order = this.props.sort_order;
    print_data.keyword = this.props.keyword;
    axios({
      url: path,
      method: "POST",
      data: { print_data },
  ...
  get_title_pdf=async()=>{
    let title = this.props.modal_title;
    let server_time = await getServerTime();
    title = title + "_" + formatDateString(new Date(server_time));
    return title + ".pdf";
  }
              ...
              {this.state.file != null ? (
                <>
                  <div>
                    ...
                    <div>
                      {/*<Document className={'flex'} file={this.state.file} onLoadSuccess={this.onDocumentLoadSuccess} renderMode={'canvas'}>*/}
                      {/*  <Page className={"PDFPage PDFPageOne"} pageNumber={pageNumber} renderTextLayer={false} renderInteractiveForms={false} renderMode={'canvas'} width={900} />*/}
                      {/*</Document>*/}
