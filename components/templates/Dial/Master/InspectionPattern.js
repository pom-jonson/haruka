        ...
        return (
            <ContextMenuUl>
                <ul className="context-menu" style={{ left: `${x}px`, top: `${y}px` }}>
                    {$canDoAction(FEATURES.DIAL_INSPECTION_PATTERN,AUTHS.EDIT,0) != false && (
                    <li><div onClick={() => parent.contextMenuAction(row_index,"edit")}>編集</div></li>
                    )}
                    {$canDoAction(FEATURES.DIAL_INSPECTION_PATTERN,AUTHS.DELETE,0) != false && (
...
class InspectionPattern extends Component {
    constructor(props) {
        super(props);
  ...
  getExamPatternItemList = async(pattern_code)=> {
    ...
    createPattern = () => {
        if (this.context.$canDoAction(this.context.FEATURES.DIAL_INSPECTION_PATTERN,this.context.AUTHS.REGISTER, 0) === false) {
    ...
    render() {
        ...
        return (
            <Card>
                <div className="title">検査項目パターン</div>
                <SearchPart>
...
InspectionPattern.contextType = Context;

export default InspectionPattern