...
class ComplicationsInspectionResult extends Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    ...
    const { data } = await axios.post(path, {params: {type:1, order:'name_kana'}});
    this.setState({
  ...
  getSearchResult = async () => {
    if(this.state.patientInfo !== null){
      ...
      let all_exam_data = this.state.all_exam_data;
  ...
  getPatternName = (code = null) => {
    ...
    if (this.state.examinationPatternList == null || this.state.examinationPatternList == undefined || this.state.examinationPatternList.length <= 0) return;
      ...
      <>
        ...
        <Card>
          <div className="d-flex">
            <div className="title">合併症検査結果一覧表</div>
            <div className={'other-pattern'}>
              {is_enabled && (
                <Button style={{marginRight:'1.5rem'}} onClick={this.gotoBedside.bind(this)}>ベッドサイド</Button>
              )}
              <Button onClick={this.goOtherPage.bind(this,"/dial/others/inspection_result")}>検査結果</Button>
              <Button className="disable-button">合併症検査結果</Button>
            </div>
          </div>
          <SearchPart>
          ...
          <div className="footer">
            ...
            <RadioButton
              ...
              checked={this.state.period_month === 24 ? true : false}
            />
...
ComplicationsInspectionResult.propTypes = {
  history:PropTypes.object,
};
export default ComplicationsInspectionResult