            ...
            <Wrapper>
                <table className="table table-bordered table-striped table-hover" id="code-table">
                    <tr >
                        ...
                        <th className="" style={{width:'2.5rem'}}>P</th>
                        <th className="ca-th" style={{width:'3rem'}}>補Ca</th>
                        <th className="">ALB</th>
                        <th className="date-area">検査日</th>
                        <th>iPTH</th>
                    </tr>
                    {table_data !== undefined && table_data !== null && table_data.length > 0 && (
                        table_data.map((item) => {
                          return (
                            <>
                              <tr onMouseOver={e => this.props.addColor(e, item.section_no, true)} 
                                onMouseOut = {e => this.props.removeColor(e, item.section_no, true)}
                                className={'record record-'+item.section_no }
                              ...
                              >
                                ...
                                <td className="text-center">{ formatDateSlash(item.examination_date) }</td>
                                <td className="text-right">{item.section_no}</td>
                                <td className="tr">{item.p}</td>
                                <td className="tr">{item.ca}</td>
                                <td className="tr">{item.alb}</td>
                                <td className="text-center">{ formatDateSlash(item.pth_examination_date)}</td>
                                {item.pth!=undefined ? (
                                    <>
                                        <td className="tr">{item.pth}</td>
                                    </>
                                ):
                                    (
                                        <>
                                            <td className={'tr'}>{'未検査'}</td>
                                        </>
                                )}
                              </tr>
                            </>
...
GuideTable.contextType = Context;
