...
class ComplicationsInspectionData extends Component {
  constructor(props) {
    super(props);
  ...
  handlePattern = (pattern_code) => {
    ...
    let nFlag = sessApi.getObjectValue('dial_change_flag', 'complications_inspection_data');
    if (nFlag == 1) {
      this.setState({
        isOpenConfirmModal: true,
  ...
  createData = () => {
    ...
    if (this.state.update_flag === 0) {
      window.sessionStorage.setItem("alert_messages", "変更されていません。");
      return;
    }
  ...
  selectPatient =async(patientInfo) => {
    ...
    // if (
    //   this.state.update_flag !== 0 ||
    //   (this.state.delete_number_list != null &&
    ...
    //   });
    // } else {
    //   this.moveOtherPatient(patientInfo);
    // }
  };
  ...
  deleteData = () => {
    if (
      this.state.delete_number_list !== undefined &&
      this.state.delete_number_list.length > 0
    ) {
      this.setState({
        isUpdateConfirmModal: true,
  ...
  render() {
    ...
    let { examinationPatternList, table_data } = this.state;
    return (
      <>
        <DialSideBar          
        ...
        />
        <Card>
          <div className="title">合併症検査データ入力</div>
          <ListTitle>
                      ...
                      return (
                        <>
                          <tr
                            ...
                            className={
                              item.code === this.state.curPatternCode
                                ? "selected"
                                : ""
                            }
                  ...
                  Object.keys(table_data).map((index) => {
                    ...
                    return (
                      <>
                        ...
                        >
                          <td className="td-no tr">{no}</td>
                          {/* <td>
                                                <Checkbox
                                                    label=""
                                                    getRadio={this.getRadio.bind(this,item.number)}
                                                    value={delete_number_list.indexOf(item.number)>-1 ? 1 : 0}
                                                    name="check"
                                                />
                                            </td> */}
                          <td className="tr code">{item.code}</td>
                          <td className="tl">{item.name}</td>
          ...
          <div className="footer-buttons">
            {/* <Button type="mono" onClick={this.inspectionCalc}>検査計算</Button> */}
              <Button className={this.state.change_flag == 0 ? "disable-btn" : "red-btn"} onClick={this.createData}>登録</Button>
            {/* <Button type="mono" onClick={this.deleteData}>一括削除</Button> */}
          </div>
...
ComplicationsInspectionData.propTypes = {    
  history: PropTypes.object
};
export default ComplicationsInspectionData;
