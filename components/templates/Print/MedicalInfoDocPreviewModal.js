...
class MedicalInfoDocPreviewModal extends Component {
	...
	async componentDidMount() {
		...
		axios({
			url: '/app/api/v2/dial/generatepdf',
			method: 'POST',
			data:{
				history_number:this.props.history_number,
				print_data:this.props.print_data,
				cur_prescription:this.props.cur_prescription,
			},
	...
	async downloadPdfB () {
		axios({
			url: '/app/api/v2/dial/generatepdfB',
			method: 'POST',
	...
	get_title_pdf = () => {		
		let pdf_file_name = "診療情報提供書(紹介状)_" + this.props.patientInfo.patient_number + "_" + formatDateLine(this.props.print_data.write_date).split("-").join("") + ".pdf";
...
MedicalInfoDocPreviewModal.propTypes = {
	...
	cur_prescription: PropTypes.string,
};
