...
class CollectionListPrintPreview extends Component {
  constructor(props) {
    super(props);
    this.state = {
      numPages: null,
  ...
  async componentDidMount() {
    let path = "/app/api/v2/dial/medicine_information/print/collection_list";
    let print_data = {};
  ...
  get_title_pdf = () => {
    let pdf_file_name = "回収表_" + this.props.schedule_date.split("-").join("") + ".pdf";
  ...
  render() {
    ...
    return  (
      <Modal show={true} id="add_contact_dlg" onHide={this.onHide} className="master-modal medical-info-doc-preview-modal first-view-modal">
        <Modal.Header>
          <Modal.Title>{'回収表'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
...
CollectionListPrintPreview.contextType = Context;

CollectionListPrintPreview.propTypes = {
...
export default CollectionListPrintPreview;