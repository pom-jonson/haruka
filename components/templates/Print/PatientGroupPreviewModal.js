...
class PatientGroupPreviewModal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      numPages: null,
  ...
  async componentDidMount() {
    let path = '/app/api/v2/dial/generatepdf/patient_grouping';
    let print_data = {};
    print_data.table_data = this.props.print_data;
    axios({
      url: path,
      method: "POST",
      data: {
  ...
  get_title_pdf = () => {
    let pdf_file_name = "患者グループ分け" + "_" + formatDateLine(this.props.schedule_date).split("-").join("") + ".pdf";
  ...
  render() {
    ...
    return (
      ...
      >
        <Modal.Header>
          <Modal.Title>患者グループ分け</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
...
PatientGroupPreviewModal.contextType = Context;

PatientGroupPreviewModal.propTypes = {
...
export default PatientGroupPreviewModal;