...
class MealInstructionList extends Component {
  constructor(props) {
    ...
    let auto_reload_info = getAutoReloadInfo("meal_prescription_list");
  ...
  async componentDidMount() {
    localApi.setValue("system_next_page", "/meal_prescription_list");
    localApi.setValue("system_before_page", "/meal_prescription_list");
  ...
  getSearchResult=async(is_searched=true, order_column=null, orderby_type=null)=>{
    ...
    let path = "/app/api/v2/meal/search/prescription_list";
    let post_data = {
  ...
  handleOrderAllPrint = () => {
    this.setState({
      ...
      confirm_type: "order_prescription_all_print",      
    });
  }
  ...
  orderPrint = (type=null) => {
    ...
    if (type === "order_prescription_all_print"){
      print_data.meal_info = this.state.pageOfItems;
    }
  ...
  confirmOk = () => {
    if(this.state.confirm_type === "order_prescription_print" || this.state.confirm_type === "order_prescription_all_print") {
                        ...
                        <>
                          ...
                          >
                            ...
                            <td style={{width:"11rem"}}>
                              {item.start_date == null ? "" : ((item.classific === "転室" ?
                                (formatJapanDateSlash(item.start_date) + " " + item.start_date.split(' ')[1]) :
                                (formatJapanDateSlash(item.start_date) + " " + item.start_time_name)
                              ))}
                            </td>
                            <td style={{width:"3rem"}}>{item.start_date == null ? "" : "より"}</td>
                            <td style={{width:"9rem"}}>{item.end_date != null ? (formatJapanDateSlash(item.end_date) + " " + item.end_time_name) : ""}</td>
                            <td style={{width:"4rem"}}>{item.classific}</td>
