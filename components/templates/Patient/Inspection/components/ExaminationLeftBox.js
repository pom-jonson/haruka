...
class ExaminationLeftBox extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getCurCategoryStatus = (nType) => {
    if (this.props.categoryType == nType) {
      return true;
    }
    return false;
  }
    ...
    switch(nType){      
      case SOAP_TREE_CATEGORY.ALL_EXAMINATION:  
        if (soapTrees.all_examination_latest != null && soapTrees.all_examination_latest!= undefined) {
          ...
          // this.changeSoapList(e, SOAP_TREE_CATEGORY.ALL_EXAMINATION_LATEST, arr[0].date.substring(0, 4), arr[0].date.substring(5, 7), arr[0].date.substring(8, 10), SOAP_TREE_CATEGORY.ALL_EXAMINATION);    
                          ...
                          {(monthItem.class_name !== undefined && monthItem.class_name.includes("open") && (
                            <ul>
                              {monthItem.data.map((data, dateIndex) => {
                                return (
                                <li key={data.number}>
                                      <span onClick={e => this.changeSoapList(e, department, itemIndex, monthItemIndex, dateIndex)} className={data.class_name !== undefined ? nOpen && dateIndex == this.props.selDay?data.class_name+" sel_open":data.class_name : ""}>{data.sdate}</span>
                                </li>
                                )
                              })}
                            </ul>
...
ExaminationLeftBox.contextType = Context;

ExaminationLeftBox.propTypes = {
...
export default ExaminationLeftBox;
