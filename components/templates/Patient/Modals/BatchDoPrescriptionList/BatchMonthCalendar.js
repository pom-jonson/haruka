...
class BatchMonthCalendar extends Component {
    constructor(props) {
        super(props);
        let cur_week = 0;
        let last_day = 0;
    ...
    setFontcolor = (day, day_of_week) => {
        ...
        if (this.props.now_date_color == true) { // if now date, background paints blue (一括Do処方一覧)
            if (formatDateSlash(new Date()) == formatDateSlash(new Date(date))) {
                return 'white';
            }
        }
    ...
    setBackgroundColor = (day) => {
        ...
        if (this.props.now_date_color == true) { // if now date, background paints blue (一括Do処方一覧)
            if (formatDateSlash(new Date()) == formatDateSlash(new Date(calendar_date))) {
                return 'dodgerblue';
            }
            if (this.props.last_prescription_date != undefined && this.props.last_prescription_date != null && this.props.last_prescription_date != "") {
                if (this.props.last_prescription_date == formatDateSlash(new Date(calendar_date))) {                    
                    return 'yellow';
                }
            }
        }
...
BatchMonthCalendar.propTypes = {
    ...
    now_date_color: PropTypes.bool,
    last_prescription_date: PropTypes.string,
};

export default BatchMonthCalendar;
