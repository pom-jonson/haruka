...
import InputNameModal from "~/components/templates/Patient/Modals/Set/InputNameModal";
...
class SetRegisterModal extends Component {
  constructor(props) {
    super(props);
  ...
  openInputNameModal=(type)=>{
    let sort_data = karteApi.getVal(this.props.patientId, CACHE_LOCALNAMES.ORDER_SORT);
    if(type === "set"){
      if(sort_data === undefined || sort_data == null || (sort_data != undefined && sort_data != null && Object.keys(sort_data).length === 0)){
        this.setState({alert_messages:"登録したオーダーがありません。"});
        return;
      }
    }
    if (type == 'set' || type == 'category') {
      if (!(this.context.$canDoAction(this.context.FEATURES.SET_REGISTER, this.context.AUTHS.REGISTER, 0))) {
        this.setState({alert_messages: "登録権限がありません。"});
        return;
      }
    }
    if (type == 'change_name') {
      if (!(this.context.$canDoAction(this.context.FEATURES.SET_REGISTER, this.context.AUTHS.EDIT, 0))) {
    ...
    this.setState({
      type,
      confirm_message:type ==="set" ? "文書名を登録しますか？" : "",
    });
    if(type ==="set"){
      return;
    }
    this.goInputnameModal();

  };

  goInputnameModal=()=>{
    let number = 0;
    let selected_key = this.state.selected_key;
    let key_arr = selected_key.split('-');
    if(key_arr[2] !== undefined){
      number = key_arr[2];  //文書のnumber
    }
    this.setState({
      openInputNameModal:true,
      number,
      confirm_message:"",
    });
  };
  ...
  deleteConfirm=()=>{
    // if(!checkMenuHasPermission(this.context.AUTHS.DELETE, 118)) {
    //     this.setState({alert_messages: "削除権限がありません。"});
    //     return;
    // }
    if (!(this.context.$canDoAction(this.context.FEATURES.SET_REGISTER, this.context.AUTHS.DELETE, 0))) {
      this.setState({alert_messages: "削除権限がありません。"});
      return;
    }
    this.setState({
      type:"delete_set",
      confirm_message:"文書を削除しますか？"});
  };
  ...
  deleteSet=async()=>{
    let selected_key = this.state.selected_key;
    let key_arr = selected_key.split('-');
    let path = "/app/api/v2/set/delete_set";
    let post_data = {
      number: key_arr[2],  //文書のnumber
    };
    await apiClient
    ...
    return (
      <>
        <Modal show={true} className="custom-modal-sm patient-exam-modal set-modal first-view-modal">
          <Modal.Header><Modal.Title>セット登録</Modal.Title></Modal.Header>
          <Modal.Body>
            <Wrapper>
                                                        ...
                                                        if (oxygen_index > 0 && oxygen_data[oxygen_index - 1].oxygen_inhaler_name == oxygen_item.oxygen_inhaler_name) oxygen_inhaler_name = "";
                                                                return (
                                                                  <div key={oxygen_item}>
                                                                    ...
                                                                    {oxygen_item.fio2_value * 100 !== undefined && oxygen_item.fio2_value != "" && (
          ...
          <Modal.Footer>
            ...
            {(this.state.selected_key !== "" && this.state.selected_key !== "other_department:0") && (
              <>
                {this.state.selected_key.split('-')[2] === undefined ? (
                  <>
                    {this.state.selected_key.split('-')[1] !== undefined ? (
                      <Button className={'disable-btn'}>分類作成</Button>
                    ):(
                      <Button className={'red-btn'} onClick={this.openInputNameModal.bind(this, 'category')}>分類作成</Button>
                    )}
                    <Button className={'red-btn'} onClick={this.openInputNameModal.bind(this, 'set')}>文書登録</Button>
                  </>
                ):(
                  <>
                    <Button className={'delete-btn'} onClick={this.deleteConfirm}>文書削除</Button>
                    <Button className={'red-btn'} onClick={this.openInputNameModal.bind(this, 'change_name')}>名前変更</Button>
                  </>
                )}
              </>
            )}
          </Modal.Footer>
          {this.state.openInputNameModal && (
            <InputNameModal
              closeModal={this.closeModal}
              type={this.state.type}
              patientId={this.props.patientId}
              number={this.state.number}
              register_category={this.state.register_category}
              register_key={this.state.register_key}
...
SetRegisterModal.contextType = Context;
SetRegisterModal.propTypes = {
  closeModal: PropTypes.func,
  patientId: PropTypes.number,
  patientInfo: PropTypes.array,
};

export default SetRegisterModal;
