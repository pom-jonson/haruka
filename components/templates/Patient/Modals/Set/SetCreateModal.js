...
class SetCreateModal extends Component {
  constructor(props) {
    super(props);
  ...
  openInputNameModal=(type)=>{
    ...
    if (type == 'set' || type == 'category') {
      if (!(this.context.$canDoAction(this.context.FEATURES.SET_CREATE, this.context.AUTHS.REGISTER, 0))) {
        this.setState({alert_messages: "登録権限がありません。"});
        return;
      }
    }
    if (type == 'change_name') {
      if (!(this.context.$canDoAction(this.context.FEATURES.SET_CREATE, this.context.AUTHS.EDIT, 0))) {
  ...
  deleteConfirm=()=>{
    if (!(this.context.$canDoAction(this.context.FEATURES.SET_CREATE, this.context.AUTHS.DELETE, 0))) {
    ...
    return (
      <>
        <Modal show={true} className="custom-modal-sm patient-exam-modal set-modal first-view-modal">
          <Modal.Header><Modal.Title>セット作成</Modal.Title></Modal.Header>
          <Modal.Body>
            <Wrapper>
                                ...
                                if(item.category_name == null){//文書
                                  return (
                                    <>
                                      <li>
                                        <span
                                          className={this.state.selected_key === "user-0"+'-'+item.number ? "selected":""}
                                          onClick={this.selectTree.bind(this, "user-0"+'-'+item.number, item)}
                                        >
                                          {item.set_name}
                                        </span>
                                      </li>
...
SetCreateModal.contextType = Context;
SetCreateModal.propTypes = {
