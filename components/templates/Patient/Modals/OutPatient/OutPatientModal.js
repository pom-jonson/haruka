...
import * as methods from "./OutPatientMethods";
...
import { CACHE_SESSIONNAMES, KARTEMODE, checkSMPByUnicode} from "~/helpers/constants";
...
import OxygenCalculateModal from "./OxygenCalculateModal";
import CountSurfaceModal from "./CountSurfaceModal";
...
import { formatJapanDateSlash, getWeekNamesBySymbol, formatDateTimeIE } from "~/helpers/date";
...
class OutPatientModal extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      isPeriodTreatment: props.from_source === "progress_chart",
      ...
      karte_status: 0,
      save_tooltip: "",
      isOpeViewSurfaceModal: false,
      timely_done: props.from_source === "progress_chart",
      start_date: new Date(),
      show_set_detail: 0,
    ...
    this.practice_width = practice_width;
  ...
  async componentDidMount() {
    ...
    let insurance_list = [];
    if (patientInfo != null && patientInfo.insurance_type_list != null && patientInfo.insurance_type_list.length > 0){
    ...
    if (this.props.cache_index == null || this.props.cache_index == undefined) {
    ...
    await apiClient._post(
      ...
      .then((res) => {
        if (res) {
          ...
          if (res.treat_init_location != undefined && res.treat_init_location != null && this.props.is_hospital == 1 && this.state.number <= 0) {
            _state.location_id = res.treat_init_location;
          }
          this.setState(_state);
    ...
    if (this.props.from_source !== "progress_chart" && this.props.cache_index != null){
  ...
  selectPractice = (id, name, enable_body_part, edit_flag=0, detail_length = 0) => {
    ...
    set_detail.map((item, index)=> {
      if (item.is_setted == 0) {
        set_detail[index] = { number: 0, check: 0, classfic: 0, classfic_name:'', item_id: 0, item_name: "", count: "", unit_id: 0, unit_name: "", main_unit: "", lot: "", comment: '' };
      }
    });
    if (this.state.practice_id != id) state_data['timely_done'] = 0;
  ...
  getDate = value => {
    ...
    if (this.props.is_hospital == 1 && this.state.isPeriodTreatment == true) return;
  ...
  save = (done_flag=0) =>{
    ...
    if (!this.context.$canDoAction(this.context.FEATURES.TREATORDER, this.context.AUTHS.DONE_OREDER) && done_flag === 1){
      return;
    }
  ...
  saveData = async (done_flag=0) =>{
    ...
    if (this.props.from_source === "progress_chart") {
      if (typeof result === "string" && result !== "success") {
        ...
        await apiClient._post(path, {params: result}).then(res=>{
  ...
  setSearchType = (e) =>{
    if (this.props.from_source === "progress_chart") return;
  ...
  openPracticeSelectModal = () => {
    if (this.props.from_source === "progress_chart") return;
    this.setState({isPracticeSelectModal:true});
  };
  ...
  getClassificationName = (classification_id) => {
    ...
    let classific_name = '';
    if (all_data.treat_classification == undefined || all_data.treat_classification == null) return '';
  ...
  handleEdit = (e, index) => {
    if (e.type === "contextmenu") {
      ...
      let x_val = e.clientX - $('#outpatient').offset().left;
  ...
  contextMenuAction = (type, index) => {
    ...
    if(type === 'edit'){
      ...
      this.selectPractice(cur_edit_data.practice_id, cur_edit_data.practice_name, cur_edit_data.enable_body_part, 1, cur_edit_data.treat_detail_item.length);
      ...
      this.setState({
        ...
        administrate_period: cur_edit_data.administrate_period != undefined && cur_edit_data.administrate_period != null ? cur_edit_data.administrate_period : null,
        ...
        timely_done: cur_edit_data.timely_done,
        quantity: cur_edit_data.quantity,
        start_date: new Date(cur_edit_data.start_date)
      });
    } else if (type == 'delete') {
      ...
      let {set_detail} = this.state;
      set_detail.push({ number: 0, check: 0, classfic: 0, classfic_name:'', item_id: 0, item_name: "", count: "", unit_id: 0, unit_name: "", main_unit: "", lot: "", comment: '' });
      this.setState({set_detail});
  ...
  changeResultEdit =(act)=>{
    ...
    if(act === "save"){
      ...
      if (this.props.is_hospital == 1 && this.state.isPeriodTreatment == true && this.state.administrate_period == null && !this.state.timely_done) {
        this.setState({alert_messages: '処置期間を選択してください。'})
        return;
      } 
      ...
      if (this.props.is_hospital == 1 && this.state.isPeriodTreatment == true) {
        detail_json.administrate_period = this.state.administrate_period;
      }
  ...
  setPeriodTreatment = (name) => {
    if (!this.canCheckPeriodTreatment() || this.state.isForUpdate == 1) {
      this.setState({isPeriodTreatment: this.state.isPeriodTreatment});
      return;
    }

    if (name == "period_check") {      
      
      let _state = {};
      if (this.state.isPeriodTreatment == true) _state.administrate_period = null;
      _state.isPeriodTreatment = !this.state.isPeriodTreatment;
  ...
  setTimeDone = () => {
    this.setState({timely_done: !this.state.timely_done});
  }
  ...
  canCheckPeriodTreatment = () => {
    let result = true;
    if (this.props.from_source === "progress_chart") return false;
    if (this.state.detail_json_array.length > 0) {
      this.state.detail_json_array.map(item=>{
  ...
  addCheck = () => {
    let result = true;
    let master_data = this.state.all_data;
    let treat_practice_master = master_data.treat_practice;
    if (this.state.detail_json_array.length > 0) {
      this.state.detail_json_array.map(item=>{
        let find_practice = treat_practice_master.find(x=>x.practice_id == item.practice_id);
        if (find_practice !== undefined && find_practice.performed_multiple_times_type == 1) result = false;
      });
    }
    return result;
  }
  ...
  openTable = () => {
    if (this.state.can_open == 0 || this.state.show_set_detail == 1) return;
    this.setState({show_set_detail: 1});
  }
  closeTable = () => {
    if (this.state.can_open == 0 || this.state.show_set_detail == 0) return;
  ...
  render() {
    ...
    let cost_width = this.cost_width;
    let practice_width = this.practice_width;
    let done_tooltip = this.doneCheck();
    if (this.props.from_source === "progress_chart") done_tooltip = "実施できません";
    ...
    return (
      <>
        <Modal show={true} id="outpatient" className="custom-modal-sm patient-exam-modal outpatient-modal first-view-modal">
          <Modal.Header><Modal.Title>{this.props.is_hospital == 1 ? "入院処置" : "外来処置"}</Modal.Title></Modal.Header>
          <Modal.Body>
              ...
              {this.state.is_loaded ? (
                <Wrapper className="treatment-modal-wrapper">
                  <div className="header-first">
                    ...
                    <div className={`date-select no-border ${this.props.is_hospital == 1 && this.state.isPeriodTreatment == true ? "no-datepicker" : ""}`}>
                      <DatePicker
                        locale="ja"
                    ...
                    <div className="ml-1 select-exit-time">
                      {this.props.is_hospital == 1 && (
                        <Checkbox
                          label="定期処置"                        
                          getRadio={this.setPeriodTreatment.bind(this)}
                          value={this.state.isPeriodTreatment}
                          name="period_check"                        
                          isDisabled={this.canCheckPeriodTreatment() != true}
                        />                      
                      )}
                      ...
                      return (
                        <>
                          <Radiobox
                            ...
                            name={`search_type`}
                            isDisabled={index == 1 && this.props.from_source === "progress_chart"}
                          />
                        </>
                      );
                    })}
                  ...
                  <div className="flex">
                    <div className="flex inline-radio">
                      ...
                      <RadioButton
                        ...
                        isDisabled={this.props.from_source === 'progress_chart'}
                      />
                    </div>
                    <div className="select-box">
                      <SelectorWithLabel
                    ...
                    <Button className="ml-2 practice-search-btn" isDisabled={this.props.from_source === "progress_chart"} onClick={this.openPracticeSelectModal.bind(this)}>行為名検索</Button>
                  </div>
                  ...
                  <div className="flex set-done mt5 w-100">
                  ...
                  <>
                    ...
                    <table className="table-scroll table table-bordered mt5" id="code-table">
                      ...
                      <tr className={'table-menu'}>
                        ...
                        <td className="text-center" style={{width:"155px"}}>数量</td>
                        <td className="text-center" style={{width:"105px"}}>単位</td>
                        <td className="text-center" style={{width:"106px"}}>ロット</td>
                        <td className="text-center" style={{width:"151px"}}>フリーコメント</td>
                      </tr>
                      </thead>
                  ...
                  <div className="flex comment-area mt5">
                    <label className="input-title">処置フリーコメント<br />（全角100文字まで）</label>
                    <textarea
                    ...
                    {this.props.is_hospital == 1 && this.state.isPeriodTreatment == true && (
                      <>
                        <Button type="common" onClick={this.setAdministratePeriod.bind(this)} className={`ml-5 period-button ${this.state.timely_done ? "disable-btn":""}`} isDisabled={this.state.timely_done}>処置期間</Button>
                        {this.state.administrate_period != undefined && this.state.administrate_period != null && (
                          <div className="flex between option table-row" style={{border:"1px solid #aaa", overflowY:"auto"}}>
                            <div className="text-right table-item" style={{width: "30rem",paddingLeft:"15px"}}>
                              <div style={{paddingRight:"5px", borderRight:"1px solid #aaa"}}>
                                1日{this.state.administrate_period.done_count}回 : {this.getDoneTimes(this.state.administrate_period.done_times)}
                              </div>
                              <div style={{paddingRight:"5px", borderRight:"1px solid #aaa"}}>
                                投与期間 : {formatJapanDateSlash(this.state.administrate_period.period_start_date)}～{formatJapanDateSlash(this.state.administrate_period.period_end_date)}
                              </div>
                              {this.state.administrate_period.period_type == 0 && this.state.administrate_period.period_category != null && (
                                <div style={{paddingRight:"5px", borderRight:"1px solid #aaa"}}>
                                  間隔 : {this.state.administrate_period.period_category == 0 ? "日":this.state.administrate_period.period_category == 1 ? "週" : "月"}
                                </div>
                              )}
                              {this.state.administrate_period.period_type == 1 && this.state.administrate_period.period_week_days.length > 0 && (
                                <div style={{paddingRight:"5px", borderRight:"1px solid #aaa"}}>
                                  曜日 : {getWeekNamesBySymbol(this.state.administrate_period.period_week_days)}
                                </div>
                              )}  
                              {this.state.administrate_period.start_count != undefined && this.state.administrate_period.done_days != undefined && ( this.state.administrate_period.start_count != 1 || this.state.administrate_period.end_count != this.state.administrate_period.done_count) && (
                                <>
                                  <div style={{paddingRight:"5px", borderRight:"1px solid #aaa"}}>
                                    初回 {formatJapanDateSlash(this.state.administrate_period.done_days[0])}の{this.state.administrate_period.start_count}回目から
                                  </div>
                                  <div style={{paddingRight:"5px", borderRight:"1px solid #aaa"}}>
                                    最終 {formatJapanDateSlash(this.state.administrate_period.done_days[this.state.administrate_period.done_days.length - 1])}の{this.state.administrate_period.end_count}回目まで
                                  </div>
                                </>
                              )}
                            </div>
                            <div className="w80 table-item" style={{width:"4rem", textAlign:"right", paddingRight:"5px"}}>
                              {this.state.administrate_period.days != undefined && this.state.administrate_period.days > 0 ? this.state.administrate_period.days+"日分":""}
                            </div>
                          </div>
                        )}
                        {this.state.select_practice !== undefined && this.state.select_practice.performed_multiple_times_type == 1 ? (
                          <div className="check-area">
                          <Checkbox
                            label="適時実施"
                            getRadio={this.setTimeDone.bind(this)}
                            value={this.state.timely_done}
                            name="period_check"
                            isDisabled={this.state.detail_json_array.length > 0}
                          />
                          </div>
                        ):(<></>)}
                        {this.state.select_practice !== undefined && this.state.select_practice.performed_multiple_times_type == 1 && this.state.timely_done ? (
                          <>
                            <div className="ml-2 mr-2" style={{lineHeight: "2rem"}}>開始日</div>
                            <DatePicker
                              locale="ja"
                      ...
                      {this.state.result_edit === false ? (
                        <>
                          {this.addCheck() ? (
                            ...
                            <Button className="ml-2 w-25" isDisabled={true} style={{opacity: 0.7}}>↓ 追加</Button>
                          )}
                        </>
                      ):(
                        <>
        ...
        {this.state.isOpeViewSurfaceModal && (
          <CountSurfaceModal
            closeModal={this.closeModal}
            handleOk={this.surfaceOk}
            modal_data={this.state.view_surface_data}
            view_only={true}
          />
        )}
...
OutPatientModal.contextType = Context;
OutPatientModal.propTypes = {
  patientId: PropTypes.number,
  patientInfo: PropTypes.object,
  closeModal: PropTypes.func,
  cache_index:PropTypes.number,
  cache_data:PropTypes.object,
  is_hospital:PropTypes.number,
