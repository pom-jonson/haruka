...
class RadiationStatusListModal extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  downLoadCsv=()=>{
    if(this.state.status_list != null && Object.keys(this.state.status_list).length > 0){
      ...
      if(this.state.view_mode === 1){
        aTag.download = '放射線予約状況一覧（詳細）.csv';
      } else {
        aTag.download = '放射線予約状況一覧（簡易）.csv';
      }
  ...
  render() {
    return (
      <>
        ...
        >
          <Modal.Header><Modal.Title>放射線予約状況一覧</Modal.Title></Modal.Header>
          <Modal.Body>
...
RadiationStatusListModal.contextType = Context;
RadiationStatusListModal.propTypes = {
  closeModal: PropTypes.func,
};

export default RadiationStatusListModal;