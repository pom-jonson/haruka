...
import SearchConditionSelectModal from "~/components/templates/Patient/Modals/Physiological/SearchConditionSelectModal";
import DepartmentResultPreviewModal from "~/components/templates/Patient/Modals/Physiological/DepartmentResultPreviewModal";
...
class RadiationDepartmentResultModal extends Component {
  constructor(props) {
    super(props);
  ...
  async UNSAFE_componentWillMount () {
    ...
    await apiClient
      ...
      .then((res) => {
        ...
        if(res.length > 0){
          res.map(radiation=>{
            order_kinds.push({id:radiation.radiation_order_id, value:radiation.name});
          });
        }
        this.setState({
          order_kinds,
  ...
  searchData=async()=>{
    ...
    let path = "/app/api/v2/order/radiation/getDepartmentResult";
    let post_data = {
    ...
    return (
      <>
        <Modal show={true} className="custom-modal-sm patient-exam-modal first-view-modal inspection-department-result-modal">
          <Modal.Header><Modal.Title>科別統計（放射線）</Modal.Title></Modal.Header>
          <Modal.Body>
            <Wrapper>
              <div className={'flex'}>
...
RadiationDepartmentResultModal.contextType = Context;
RadiationDepartmentResultModal.propTypes = {
    closeModal: PropTypes.func,
};

export default RadiationDepartmentResultModal;
