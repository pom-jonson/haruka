...
import PeriodViewListModal from "~/components/templates/Patient/Modals/Physiological/PeriodViewListModal";
...
class EndoscopeStatusListModal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      if(data['order'] !== undefined && data['order'][i] !== undefined){
        ...
        table_data.push(
          <div className={'patient-box'}>
            ...
            <span>{this.state.frame_name}</span>
          </div>
        );
      } else {
        table_data.push(
  ...
  downLoadCsv=()=>{
    if(this.state.status_list != null && Object.keys(this.state.status_list).length > 0){
      ...
      if(this.state.view_mode === 1){
        aTag.download = '内視鏡検査予約状況一覧（詳細）.csv';
      } else {
        aTag.download = '内視鏡検査予約状況一覧（簡易）.csv';
      }
  ...
  render() {
    return (
      <>
        ...
        >
          <Modal.Header><Modal.Title>内視鏡検査予約状況一覧</Modal.Title></Modal.Header>
          <Modal.Body>
...
EndoscopeStatusListModal.contextType = Context;
EndoscopeStatusListModal.propTypes = {
  closeModal: PropTypes.func,
};

export default EndoscopeStatusListModal;