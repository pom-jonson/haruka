...
class InspectionListModal extends Component {
    ...
    searchList = async () => {
        ...
        let post_data = {
            ...
            group_code:this.props.conditions.group_code,
    ...
    printPdf=async()=>{
        if(this.state.list_data != null && this.state.list_data.length > 0){
            let base_modal = document.getElementsByClassName("endoscope-list-modal")[0];
            ...
            print_data.inspection_name = '内視鏡';
            ...
            axios({
                ...
                if(window.navigator.msSaveOrOpenBlob) {
                    //IE11 & Edge
                    window.navigator.msSaveOrOpenBlob(blob, '内視鏡検査予定一覧.pdf');
                }
                else{
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', '内視鏡検査予定一覧.pdf'); //or any other extension
    ...
    download_csv=()=> {
        if(this.state.list_data != null && this.state.list_data.length > 0){
            ...
            this.state.list_data.map(item=>{
                ...
                let data_item = [
                    item.order_data.order_data.inspection_name,
    ...
    render() {
        return (
            <>
                <Modal
                    show={true}                    
                    className="custom-modal-sm patient-exam-modal physiological-modal endoscope-list-modal"
                >
                                        ...
                                        this.state.list_data.length > 0 ? (
                                            this.state.list_data.map((item, key) => {
                                                return (
                                                    <>
                                                        <tr key={key}>
                                                            <td className={'text-center'} style={{width:"200px"}}>{item.order_data.order_data.inspection_name}</td>
