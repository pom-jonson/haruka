...
class KarteDischargeHospital extends Component {
  constructor(props) {
    super(props);
    this.state = {
      load_flag:false,
  ...
  getDischargePermit = async (cache_exit) => {
    ...
    await apiClient
      ...
      if(res.error_message !== undefined){
        ...
        } else if(res.hos_data.expected_discharge_date != null){
          discharge_date = new Date(res.hos_data.expected_discharge_date.split("-").join("/"));
        }
        if(discharge_date.getTime() < new Date().getTime()){
          discharge_date = new Date();
        }
        if(res.moving_day !== undefined){
          let final_moving_day = new Date(res.moving_day);
          if(final_moving_day.getTime() > this.discharge_date.getTime()){
            this.discharge_date = final_moving_day;
            discharge_date = final_moving_day;
          }
        }
  ...
  register = async () => {
    ...
    // save to cache
    karteApi.setVal(this.props.patientId, CACHE_LOCALNAMES.DISCHARGE_DONE, JSON.stringify(discharge_done), 'insert');
    this.context.$setExaminationOrderFlag(1);
    this.props.closeModal();
  }

  loadFromCache = () => {
    let cache_data = karteApi.getVal(this.props.patientId, CACHE_LOCALNAMES.DISCHARGE_DONE);
              ...
              {this.state.load_flag ? (
                <div style={{paddingTop:"40px", paddingLeft:"60px"}}>
                  <div className={'flex select-date'}>
                    ...
                    {(this.discharge_date.getTime() > new Date().getTime()) ? (
...
KarteDischargeHospital.contextType = Context;

KarteDischargeHospital.propTypes = {
  closeModal: PropTypes.func,
  patientId : PropTypes.number,
};

export default KarteDischargeHospital;
