...
export default async function(params, init=false, first_load=null) {
  ...
  if (data) {
    let addCount = 0;
    medicineHistory = data.filter(item => {
      if (
  ...
  let state_data = {};
  state_data.medicineHistory = medicineHistory;
  state_data.medicineDBHistory = data;
  await this.getHistoryMoreData(params, !init, first_load).then(function(data){
