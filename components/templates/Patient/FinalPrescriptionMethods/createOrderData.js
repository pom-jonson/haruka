...
export default function(source) {
  let keys = Object.keys(this.state.usageData);
  keys.map(key => {
    let subKeys = Object.keys(this.state.usageData[key]);
    subKeys.map(subKey => {
      source = source.map(pres => {
        this.state.usageData[key][subKey].map(item => {
  ...
  let results = source
    .filter(pres => {
      ...
      return (
        pres.usage > 0 &&
        (pres.days > 0 ||
          (pres.usage_replace_number !== undefined &&
            pres.usage_replace_number.length > 0))
      );
    })
    .map(pres => {
