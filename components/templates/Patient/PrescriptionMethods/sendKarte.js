...
export default async function(patient_id, next_page = "") {
  ...
  let existDeleteDocumentState = false;
  if(cacheDelDocumentState != undefined && cacheDelDocumentState != null && Object.keys(cacheDelDocumentState).length > 0){
    existDeleteDocumentState = true;
  }
  // get Rehabily delete
    ...
    await apiClient
      ...
      .then((res) => {
        if(res.error_message != undefined){
          ...
          if(cacheHospitalDoneState.department_id != this.context.department.code){
            this.context.$updateDepartment(cacheHospitalDoneState.department_id, cacheHospitalDoneState.department_name, patient_id);
          }
    ...
    for (let i = 0; i < arr.length; i ++) {
      ...
      if (orderData[0] !== undefined && orderData[0].med.length != 0) {
        ...
        if (!isUsageCommentError) {
          ...
          if (cur_cache_data[0].increasePeriod != undefined && cur_cache_data[0].increasePeriod == 1) {
            postData.increasePeriod = 1;
          }
          ...
          if (patientInfo != undefined && patientInfo != null && patientInfo.visit_info != undefined && patientInfo.visit_info != null) {
            ...
            if (visit_info.visit_place_id != undefined && visit_info.visit_place_id != null && visit_info.visit_place_id > 0 && karte_status == 2) {
              visit_place_id = visit_info.visit_place_id;              
            }
          }
          if (visit_place_id != 0 && isForPrescriptionUpdate != 1) {            
            postData.visit_place_id = visit_place_id;            
          }
        ...
        if (_state_presData.length == 1 && _state_presData[0].medicines.length == 1 && deleted){
          // 何も入力せず削除した場合はアラートなし
        } else{
          strMessage = "用法方法を入力して下さい。";
          this.addMessageSendKarte(step, type, strMessage, 1);
  ...
  if (error == false && existCacheInjectState == true) {
    ...
    for (let i = 0; i < arr.length; i ++) {
      ...
      if (visit_place_id > 0 && isForInjectionUpdate != 1) {
        postData.visit_place_id = visit_place_id;        
      }
      if (this.state.staff_category === 2 || this.context.$canDoAction(this.context.FEATURES.PRESCRIPTION, this.context.AUTHS.REGISTER_PROXY)) {
        postData.doctor_name = this.context.selectedDoctor.name;
  ...
  if(error == false && existCacheSoapState == true){
    ...
    if(cacheSoapState.isForUpdate){
      if(soap.number == -1){
        soap.isNew_Updated = true;
      } else {
        soap.isUpdated = true;
      }
    ...
    if (patientInfo != undefined && patientInfo != null && patientInfo.visit_info != undefined && patientInfo.visit_info != null) {
      ...
      if (visit_info.visit_place_id != undefined && visit_info.visit_place_id != null && visit_info.visit_place_id > 0 && karte_status == 2) {
        visit_place_id = visit_info.visit_place_id;
        visit_place_name = visit_info.place_name;
      }
    }
    if (visit_place_id != 0) {
      soap.visit_place_id = visit_place_id;
      soap.visit_place_name = visit_place_name;
    }
  ...
  if (existExamState) {
    ...
    if (visit_place_id != 0) {
      cacheExamState.visit_place_id = visit_place_id;
      cacheExamState.visit_place_name = visit_place_name;
    }
  ...
  if (existCytologyExamState) {
    ...
    is_seal_print = karteApi.getSealSubVal(patient_id, CACHE_LOCALNAMES.CYTOLOGY_EXAM_EDIT);
    let patientInfo = karteApi.getPatient(patient_id);
    ...
    if (visit_place_id != 0) {
      cacheCytologyExamState.visit_place_id = visit_place_id;
      cacheCytologyExamState.visit_place_name = visit_place_name;
    }
    await apiClient
      ._post(path, {
  ...
  if (existPathologyExamState) {
    ...
    is_seal_print = karteApi.getSealSubVal(patient_id, CACHE_LOCALNAMES.PATHOLOGY_EXAM_EDIT);
    let patientInfo = karteApi.getPatient(patient_id);
    ...
    if (visit_place_id != 0) {
      cachePathologyExamState.visit_place_id = visit_place_id;
      cachePathologyExamState.visit_place_name = visit_place_name;
    }
    await apiClient
      ._post(path, {
  ...
  if (existBacterialExamState) {
    ...
    is_seal_print = karteApi.getSealSubVal(patient_id, CACHE_LOCALNAMES.BACTERIAL_EXAM_EDIT);
    let patientInfo = karteApi.getPatient(patient_id);
    ...
    if (visit_place_id != 0) {
      cacheBacterialExamState.visit_place_id = visit_place_id;
      cacheBacterialExamState.visit_place_name = visit_place_name;
    }
    await apiClient
      ._post(path, {
  ...
  // inspection save
  if (error == false && existCacheInspectionState){
    const authInfo = JSON.parse(window.sessionStorage.getItem("haruka"));
    if (authInfo.doctor_code === 0) {
      if (this.context.selectedDoctor.code === 0) {
        strMessage = "依頼医を選択してください";
    ...
    for (let i = 0; i < arr.length; i ++) {
      ...
      cur_cache_data.doctor_code = authInfo.staff_category == 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
      cur_cache_data.doctor_name = authInfo.staff_category == 1 ? authInfo.name : this.context.selectedDoctor.name;
      let karte_status = 1;
      ...
      if (visit_place_id != 0) {
        cur_cache_data.visit_place_id = visit_place_id;
        cur_cache_data.visit_place_name = visit_place_name;
      }
      await apiClient
        ._post(path, {
  ...
  if (error == false && existcacheDischargePermitState == true) {
    ...
    cacheDischargePermitState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheDischargePermitState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
  ...
  if (error == false && existcacheChangeResponsibilityState == true) {
    ...
    cacheChangeResponsibilityState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheChangeResponsibilityState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
    await apiClient
      ._post(path, {
        params: cacheChangeResponsibilityState
      })
      .then(() => {
        if(cacheChangeResponsibilityState.department_id != this.context.department.code){
          this.context.$updateDepartment(cacheChangeResponsibilityState.department_id, cacheChangeResponsibilityState.department_name, patient_id);
        }
  ...
  if (error == false && existcacheInHospitalState == true) {
    ...
    cacheInHospitalState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheInHospitalState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
    ...
    await apiClient
      ...
      .then((res) => {
        if(res.error_message == undefined){
          ...
          if(cacheInHospitalState.department_id != this.context.department.code){
            this.context.$updateDepartment(cacheInHospitalState.department_id, cacheInHospitalState.department_name, patient_id);
          }
  ...
  if (error == false && existcacheNutritionGuidanceState == true) {
    ...
    cacheNutritionGuidanceState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheNutritionGuidanceState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
    await apiClient
      ._post(path, {
  ...
  if (error == false && existcacheMedicalExaminationRecordState == true) {
    ...
    cacheMedicalExaminationRecordState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheMedicalExaminationRecordState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
    await apiClient
      ._post(path, {
  ...
  if (error == false && existcacheDischargeDoneState == true) {
    ...
    cacheDischargeDoneState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheDischargeDoneState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
  ...
  if (error == false && existcacheDischargeDecisionState == true) {
    ...
    cacheDischargeDecisionState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheDischargeDecisionState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
  ...
  //document delete
  if(existDeleteDocumentState) {
    let arr = [];
    Object.keys(cacheDelDocumentState).forEach(function(key){
      arr.push(key);
    });
    for (let i = 0; i < arr.length; i++) {
      let path = "/app/api/v2/document/delete_file_info";
      let document = cacheDelDocumentState[arr[i]];
      let post_data = {
      ...
      await apiClient
        ...
        .then(() => {
          karteApi.delSubVal(patient_id, CACHE_LOCALNAMES.DOCUMENT_DELETE, arr[i]);
        })
        .catch(() => {
          return false;
        });
    }
    karteApi.delVal(patient_id, CACHE_LOCALNAMES.DOCUMENT_DELETE);
  }
  ...
  if (error == false && existcacheDischargeGuidanceReportState == true) {
    ...
    cacheDischargeGuidanceReportState.doctor_code = authInfo.staff_category === 1 ? authInfo.doctor_code : this.context.selectedDoctor.code;
    cacheDischargeGuidanceReportState.doctor_name = authInfo.staff_category === 1 ? authInfo.name : this.context.selectedDoctor.name;
    let discharge_decision_error = false;
    await apiClient
      ._post(path, {
