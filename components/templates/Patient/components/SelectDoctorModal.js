...
export class SelectDoctorModal extends Component {
    ...
    setDoctors = (doctors) => {
        ...
        let startIndex = 0;
        //check from regular injection page and set main doctor as default---------------
        let defalut_department_name = "内科";
        if (
            this.props.from_regular_injection == true && 
            this.main_doctor_id > 0
        ){
            defalut_department_name = '主治医';
        }
        //---------------------------------------------------------------------------------
        tabs.map((tab, index) => {
            if (tab.name === defalut_department_name) startIndex = index;
        });

        this.setState({ tabs: tabs, tab: startIndex }, function() {
            if (defalut_department_name == '主治医'){
                var main_doctors = this.state.doctors.filter(x => x.doctor_code == this.main_doctor_id);
                this.setState({showDoctors:main_doctors})
            } else {
    ...
    render() {
        return (
            ...
            >
                ...
                <Modal.Footer>
                    <Button className="cancel-btn" onClick={this.closeDoctor}>キャンセル</Button>
                </Modal.Footer>
            </Modal>
        );
    }
...
SelectDoctorModal.propTypes = {
    ...
    from_regular_injection:PropTypes.bool,
};
