export default async function(order_number, is_preset = false) {
  ...
  this.state.injectionHistory.map(injection => {
    injection.order_data.order_data.map((item, idx) => {
  ...
  if (order !== undefined) {
    ...
    if (this.checkHasNoUnitInjectionOrder(order, order_idx) == false) {
      return;
    }
    ...
    //     return true;
    //   } else {
      let checkDisease = await this.checkDiseaseOrderInjection(order);
      if (!checkDisease) {
          if (!this.checkPeriodMedicineInjection(order)) {
