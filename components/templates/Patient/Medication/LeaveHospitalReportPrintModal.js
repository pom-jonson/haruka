...
class LeaveHospitalReportPrintModal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      numPages: null,
  ...
  async componentDidMount() {
    // download pdf
    var url;
    url = '/app/api/v2/print_haruka/generatepdf/leave_hospital_report';
  ...
  downloadPdf =()=>{
    if(this.state.file != null){
      ...
      if(window.navigator.msSaveOrOpenBlob) {
        //IE11 & Edge
        window.navigator.msSaveOrOpenBlob(blob, '退院時指導レポート.pdf');
      }
      else{
        const url = window.URL.createObjectURL(new Blob([this.state.file]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', '退院時指導レポート.pdf'); //or any other extension
  ...
  render() {
    ...
    return  (
      <Modal show={true} onHide={this.onHide} id="add_contact_dlg_print"  className="medical-info-doc-preview-modal blood-record-modal">
        <Modal.Header>
          <Modal.Title>退院時指導レポート</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
...
LeaveHospitalReportPrintModal.contextType = Context;

LeaveHospitalReportPrintModal.propTypes = {
  closeModal: PropTypes.func,
  handleOk : PropTypes.func,
  print_data:PropTypes.object
};

export default LeaveHospitalReportPrintModal;
