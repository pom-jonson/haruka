...
class InhomeClassificModal extends Component {
    constructor(props) {
        super(props);
    ...
    async registerMaster()  {
        ...
        switch(this.props.kind){
            case 0:
                path = "/app/api/v2/master/inhome/registerClassific";
                break;
            case 1:
                path = "/app/api/v2/master/inhome/registerClassificDetail";
                break;            
        }
    ...
    render() {        
        return  (
            <Modal show={true} id="add_contact_dlg"  className="medicine-modal first-view-modal">
                <Modal.Header>
                    <Modal.Title>在宅{this.state.title}マスタ{this.props.modal_data != null?'編集':'登録'}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Wrapper>
...
InhomeClassificModal.contextType = Context;

InhomeClassificModal.propTypes = {
...
export default InhomeClassificModal;
