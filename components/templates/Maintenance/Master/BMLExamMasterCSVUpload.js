...
class BMLExamMasterCSVUpload extends React.Component {
  constructor(props) {
    super(props);
    this.state ={
      file: null,
  ...
  register=()=>{
    ...
    axios.post("/app/api/v2/karte/bml_exam_master/csv_upload", this.state.formData, this.state.config)
      .then((response) => {
        this.setState({
          complete_message:"",
        });
        if(response.data.error_message !== undefined){
          ...
          // window.sessionStorage.setItem("alert_messages", response.data.error_message + "\n");
        }
        ...
        <Wrapper>
          ...
          <FormUpload>
            <form onSubmit={this.onFormSubmit}>
              <div className='flex'>
                <label style={{paddingTop:'4px'}}>BML検体検査結果CSVファイル</label>
...
BMLExamMasterCSVUpload.contextType = Context;
BMLExamMasterCSVUpload.propTypes = {
  history: PropTypes.object,
}
export default BMLExamMasterCSVUpload