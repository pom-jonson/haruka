                  ...
                  <li>
                      <div className="tooltip-content-area">
                          <div className="tooltip-item">                              
                              <div className="item-content">{tooltip_content}</div>
                          </div>
                      </div>                                                
                  </li>
              </ul>
...
class StaffOperationLog extends Component {
    constructor(props) {
        super(props);
    ...
    getStaffLogs = async (_flag=null) => {
      let path = "/app/api/v2/dial/staff/search_staff_logs";
        var post_data = {            
                ...
                <SearchPart>
                    ...
                    <div className={'flex search-part'} style={{paddingTop:"10px"}}>
                        <div className='direct_man flex' >
                          ...
                          onClick={(e)=>this.showPatientList(e).bind(this)}
                          >
                            <InputWithLabel
                              label="患者指定"
                              type="text"
                  ...
                  <table className="table table-bordered table-hover">
                      <thead style={{borderBottom:"1px solid #dee2e6"}}>
                          <tr style={{width:"calc(100% - 17px)"}}>                            
                              <th className="code-number">処理日</th>
                              <th className="code-number">処理時間</th>
                        ...
                        <>
                          {pageOfItems !== undefined && pageOfItems !== null && pageOfItems.length > 0 && (
                              pageOfItems.map((item, index) => {
                                  ...
                                  >
                                      <td className="code-number text-left">{item.cur_date}</td>
                                      <td className="code-number text-left">{item.op_date}</td>
                                      <td className="code-number text-left">{item.cur_date}</td>
                                      <td className="code-number text-right">{item.patient_number}</td>
                                      ...
                                      <td className="name text-left">{item.op_screen}</td>
                                      <td className="name text-left">{item.name}</td>
                                      <td className='text-left'>{item.op_type}</td>
                                  </tr>
                                  </>)
                              })
                          )}
...
StaffOperationLog.contextType = Context;
export default StaffOperationLog