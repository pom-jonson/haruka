...
export default async function(departmentIndex, yearIndex = -1, monthIndex = -1, dateIndex = -1, nCategoryType = -1) {
  ...
  // }
    ...
    // else if(departmentIndex == SOAP_TREE_CATEGORY.ALL_HOSPITAL || departmentIndex == SOAP_TREE_CATEGORY.ALL_HOSPITAL_LATEST){
    //     let allHospital = this.state.soapTrees.all_hospital.map((item, index) => {
    //         item.class_name = "";
    //         if (index === yearIndex) {
    //             item.class_name = "open";
    //         }
    //         if (nCategoryType === SOAP_TREE_CATEGORY.ALL_HOSPITAL) {
    //             item.class_name = "";
    //         }
    //         item.data = item.data.map((monthItem, ind) => {
    //             monthItem.class_name = "";
    ...
    //         }
    //     })
    //     if(departmentIndex == SOAP_TREE_CATEGORY.ALL_HOSPITAL_LATEST){
    //         active_soap_latest = this.state.soapTrees.all_hospital_latest;
    //          Object.keys(active_soap_latest).forEach(function(key){
    //             if (key == strDate) {
    //                 active_soap_latest[key].class_name="open";
    //             } else {
    ...
    //         soapList = [];
    //
    //         var stateAllHospitalLatest = this.state.soapTrees.all_hospital_latest;
    //         Object.keys(stateAllHospitalLatest).forEach(function(key){
    //             if (key == strDate) {
    //                 // soap_latest = allDateList[strDate];
    //                 soap_latest = stateAllHospitalLatest[strDate];
    //                 return false;
    //             }
    //         });
    //
    //         if (yearIndex == -1 && monthIndex == -1 && dateIndex == -1) {
    //             stateCurrentSoapLatest = this.state.soapTrees.all_hospital_latest;
    //             Object.keys(stateCurrentSoapLatest).forEach(function(key){
    //                 soap_latest.push(stateAllHospitalLatest[key]);
    //             });
    //         }
    //         if (Object.keys(soap_latest).length < 1) {
    //             return;
    ...
    //                 all_hospital_latest:departmentIndex == SOAP_TREE_CATEGORY.ALL_HOSPITAL_LATEST?active_soap_latest:this.state.soapTrees.all_hospital_latest,
    //             },
    //             soapList: this.sortSoapList(soapList),
    //             categoryType: SOAP_TREE_CATEGORY.ALL_HOSPITAL_LATEST,
    //         });
    //     }else{
    //         this.setState({
    //         soapTrees: {
