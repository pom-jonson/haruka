...
class NurseAnamuneModal extends Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    this.can_register = this.context.$canDoAction(this.context.FEATURES.ANAMNE,this.context.AUTHS.REGISTER);
    this.can_register_old = this.context.$canDoAction(this.context.FEATURES.ANAMNE,this.context.AUTHS.REGISTER_OLD);
    this.can_edit_old = this.context.$canDoAction(this.context.FEATURES.ANAMNE,this.context.AUTHS.EDIT_OLD);
  ...
  get_title_pdf =async() => {
    ...
    let pdf_file_name = "アナムネ_" + this.state.patientInfo.receId + "_" + formatDateString(new Date(server_time)) + ".pdf";
  ...
  componentDidUpdate() {
    harukaValidate('nursing', 'summary', 'anamune', this.state, 'background');
  }
  
  checkValidation = () => {
    let error_str_arr = [];
    let validate_data = harukaValidate('nursing', 'summary', 'anamune', this.state);
    ...
    return (
      <Modal show={true} id="add_contact_dlg"  className="custom-modal-sm patient-exam-modal bed-control-modal nurse-summary first-view-modal">
                    ...
                    <div className={'scroll-area'}>
                      <div className={'flex border-1px'}>
                        ...
                        <div className={'w-50-p'}>
                          ...
                          <div className={'border-bottom-1px'} style={{width:"100%", height:"calc(8rem + 3px)", overflowY:"auto"}}>
                            <textarea
        ...
        <Modal.Footer>
          ...
          <Button className={(this.change_flag == 1 && this.can_register) ? "red-btn" : 'disable-btn'} onClick={this.registerConfirm.bind(this, 1)}>保存</Button>
        </Modal.Footer>
...
NurseAnamuneModal.contextType = Context;
NurseAnamuneModal.propTypes = {
  patientId: PropTypes.number,
  patientInfo: PropTypes.object,
  closeModal: PropTypes.func,
}
export default NurseAnamuneModal;
