...
class ResultInsert extends Component {
    constructor(props) {
      super(props);
      this.state = {
        input_date: new Date(props.cur_datetime.split("-").join("/")),
        title_array: [],
        check_array:[],
        comment_check: false,
      };
        ...
        tier_master_3rd.map(item=> {
          if (item.tier_2nd_id == selected_tier.tier_2nd_id) {
            ...
            if (find_elapsed_result != undefined) {
              ...
              let input_value = this.getResultFromProps(find_elapsed_result);
              let check_item = {
                ...
                result: item.result_type == 0 && input_value != null ? input_value.result : 0,
                elapsed_result_text: item.result_type == 1 && input_value != null ? input_value.elapsed_result_text : "",
                unit:'',
                result_check: input_value != null,
                comment_check: false,
              };
    ...
    getResultFromProps = (result_data) => {
      if (result_data === undefined || result_data == null) return null;
      let input_values = result_data.input_values;
      if (input_values === undefined || input_values.length === 0) return null;
      let {range, cur_datetime} = this.props;
      cur_datetime = cur_datetime.split("-").join("/");
      if (range == 1) cur_datetime = cur_datetime + " 00:00";
      let start_datetime = new Date(cur_datetime).getTime();
      let delta = 24;
      if (range == 2) delta = 3;
      else if (range == 3) delta = 1;
      let end_datetime = start_datetime + delta * 3600 * 1000;
      let find_value = null;
      input_values.map(item=> {
        let input_datetime = item.input_datetime;
        if (input_datetime !== undefined && input_datetime != null && input_datetime !== "") {
          input_datetime = input_datetime.split("-").join("/");
          input_datetime = new Date(input_datetime).getTime();
          if (input_datetime >= start_datetime && input_datetime < end_datetime) {
            find_value = item;
          }
        }
      });
    ...
    getResult = (index, e) => {
      let {check_array} = this.state;
      if (!check_array[index].result_check) return;
      check_array[index].elapsed_result_text = e.target.value;
      this.change_flag = 1;
      this.setState({check_array});
    };
    render() {
              ...
              <DatePickerBox style={{width:"100%", height:"100%"}}>
                <Wrapper>
                  ...
                  <div className={'select-date flex'} style={{marginBottom:"0.5rem"}}>
                    <button onClick={this.moveDay.bind(this, 'prev')} style={{height:"2.3rem"}}>{'<'}</button>
                    <DatePicker
                        locale="ja"
                        id='end_date_id'
                    ...
                    />
                    <button onClick={this.moveDay.bind(this, 'next')} style={{marginLeft:"0.5rem", height:"2.3rem"}}>{'>'}</button>
                    <button className="ml-3" onClick={this.selectToday} style={{height:"2.3rem"}}>現在時刻</button>
                  </div>
                            ...
                            return (
                              ...
                              <tr key={index}>
                                ...
                                <td className='td-result'>
                                  {item.result_type == 1 ? (
                                    <input type="text" value={item.elapsed_result_text} onChange={this.getResult.bind(this, index)} disabled={!item.result_check}/>
                                  ):(
                                    <SelectorWithLabel
                                      title=""
