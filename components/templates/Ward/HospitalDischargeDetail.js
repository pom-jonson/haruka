...
class HospitalDischargeDetail extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return  (
      <Modal show={true}  className="custom-modal-sm first-view-modal haruka-done-modal">
        <Modal.Header><Modal.Title>{modal_data.sub_category}</Modal.Title></Modal.Header>
        <Modal.Body>
          <Wrapper>
              ...
              <Bar>
                <div className="content">
                  ...
                  <div className="data-list">
                    <div className={`data-title ${this.getOrderTitleClassName(modal_data.sub_category === '退院実施' ? 1 : modal_data.done_order, modal_data.karte_status)}`}>
                      <div className={'data-item'}>
                        <div className="flex justify-content">
                          <div className="note">【入院・{modal_data.sub_category}】</div>
                        ...
                        <div className="phy-box w70p">
                          <div className="flex between drug-item table-row">
                            <div className="text-left">
                              <div className="table-item">{modal_data.sub_category === '退院実施' ? "実施日時" : (modal_data.sub_category === '退院決定' ? "退院日時" : "退院日")}</div>
                            </div>
                            <div className="text-right">
                              <div className="table-item remarks-comment">
                                {modal_data.sub_category === "退院許可" ?
                                  (modal_data.order_data.order_data.discharge_date != "" && formatJapanDateSlash(modal_data.order_data.order_data.discharge_date))
                            ...
                            <div className="flex between drug-item table-row">
                              ...
                              <div className="text-right">
                                <div className="table-item remarks-comment">
                                  ...
                                  +" （"+modal_data.order_data.order_data.start_time_name+"）"}より停止
                                </div>
                              </div>
                            </div>
                          )}
                          <div className="flex between drug-item table-row">
                            ...
                            <div className="text-right">
                              <div className="table-item remarks-comment">{modal_data.order_data.order_data.outcome_reason_name}</div>
                            </div>
                          </div>
                          <div className="flex between drug-item table-row">
                            <div className="text-left">
                            ...
                            <div className="text-right">
                              <div className="table-item remarks-comment">{modal_data.order_data.order_data.discharge_route_name}</div>
                            </div>
                          </div>
                          {(modal_data.sub_category != "退院許可" && modal_data.order_data.order_data.discharge_free_comment != "") && (
                            <div className="flex between drug-item table-row">
                              <div className="text-left">
                                <div className="table-item">フリーコメント</div>
                              </div>
                              <div className="text-right">
                                <div className="table-item remarks-comment">{modal_data.order_data.order_data.discharge_free_comment}</div>
                              </div>
                            </div>
                          )}
                          {(modal_data.sub_category == "退院許可" && modal_data.order_data.order_data.free_comment != undefined && modal_data.order_data.order_data.free_comment != null &&
...
HospitalDischargeDetail.contextType = Context;
HospitalDischargeDetail.propTypes = {
  closeModal: PropTypes.func,
  modal_data: PropTypes.object,
};
export default HospitalDischargeDetail;
