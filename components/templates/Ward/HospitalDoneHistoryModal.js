...
export class HospitalDoneHistoryModal extends Component {
  constructor(props) {
    super(props);
                            ...
                            <div className={`history-item ${output.is_enabled == 2 ? "deleted-order" : ""}`}>
                              <div className="phy-box w70p">
                                ...
                                <div className="flex between drug-item table-row">
                                  ...
                                  <div className="text-left right-value">
                                    {output.prev == null ? (
                                      this.getChangePart(output.order_data.order_data.bed_name, undefined)
                                    ):(
                                      this.getChangePart(output.order_data.order_data.bed_name, output.prev.order_data.order_data.bed_name)
                                    )}
                                  </div>
                                </div>
                                <div className="flex between drug-item table-row">
                                  ...
                                  <div className="text-left right-value">
                                    {output.prev == null ? (
                                      this.getChangePart(
                                        ...
                                        (" ("+ output.order_data.order_data.start_time_classification_name +") より開始") : ""),
                                        undefined)
                                    ):(
                                      this.getChangePart(
                                        formatJapanDateSlash(output.order_data.order_data.start_date)
                                        ...
                                        (" ("+ output.prev.order_data.order_data.start_time_classification_name +") より開始") : ""))
                                    )}
                                  </div>
                                </div>
                              </div>
...
HospitalDoneHistoryModal.contextType = Context;

HospitalDoneHistoryModal.propTypes = {
  closeModal: PropTypes.func,
  getDepartmentName: PropTypes.func,
  historySoapList: PropTypes.array,
};

export default HospitalDoneHistoryModal;