...
export class DischargeHistoryModal extends Component {
  constructor(props) {
    super(props);
  ...
  checkDifference = (results) => {
    ...
    if (result.length > 0) {
      result.map((order, index) => {
        ...
        let current = order;
        current.doctor_name = order.order_data.order_data.doctor_name != undefined ? order.order_data.order_data.doctor_name : this.doctor_names[order.order_data.order_data.doctor_code];
                        ...
                        <>
                          <tr>
                            ...
                            <td className="text-left">
                              <span className={doctor_change ? "text-blue":""}>
                                {item.order_data.order_data.doctor_name !== undefined ? item.order_data.order_data.doctor_name : this.doctor_names[item.order_data.order_data.doctor_code]}
                              </span>
                              ...
                              <div className="phy-box w70p">
                                <div className="flex between drug-item table-row">
                                  <div className="text-left">
                                    <div className="table-item">{modal_title === '退院実施' ? "実施日時" : (modal_title === '退院決定' ? "退院日時" : "退院日")}</div>
                                  </div>
                                  <div className="text-left right-value">
                                    {output.prev == null ? (
                                      this.getChangePart(
                                        (modal_title === "退院許可" ?
                                          (output.order_data.order_data.discharge_date != "" && formatJapanDateSlash(output.order_data.order_data.discharge_date))
                                      ...
                                      this.getChangePart(
                                        ...
                                        (modal_title === "退院許可" ?
                                            (output.prev.order_data.order_data.discharge_date != "" && formatJapanDateSlash(output.prev.order_data.order_data.discharge_date))
                                ...
                                <div className="flex between drug-item table-row">
                                  ...
                                  <div className="text-left right-value">
                                    {output.prev == null ? (
                                      this.getChangePart(output.order_data.order_data.outcome_reason_name, undefined)
                                    ):(
                                      this.getChangePart(output.order_data.order_data.outcome_reason_name, output.prev.order_data.order_data.outcome_reason_name)
                                    )}
                                  </div>
                                </div>
                                <div className="flex between drug-item table-row">
                                  ...
                                  <div className="text-left right-value">
                                    {output.prev == null ? (
                                      this.getChangePart(output.order_data.order_data.discharge_route_name, undefined)
                                    ):(
                                      this.getChangePart(output.order_data.order_data.discharge_route_name, output.prev.order_data.order_data.discharge_route_name)
                                    )}
                                  </div>
                                </div>
                                {modal_title != "退院許可"
                                  && (output.order_data.order_data.discharge_free_comment != "" || (output.prev != null && output.prev.order_data.order_data.discharge_free_comment != ""))
                                  && (
                                  <div className="flex between drug-item table-row">
                                    <div className="text-left">
                                    ...
                                    <div className="text-left right-value">
                                      {output.prev == null ? (
                                        this.getChangePart(output.order_data.order_data.discharge_free_comment, undefined)
                                      ):(
                                        this.getChangePart(output.order_data.order_data.discharge_free_comment, output.prev.order_data.order_data.discharge_free_comment)
                                      )}
                                    </div>
                                  </div>
                                )}
                                {modal_title == "退院許可"
                                  && (
                                    (output.order_data.order_data.free_comment != undefined
                                      && output.order_data.order_data.free_comment != null
                                      && output.order_data.order_data.free_comment != ""
                                    )
                                  || (output.prev != null && output.prev.order_data.order_data.discharge_free_comment != undefined
                                    && output.prev.order_data.order_data.discharge_free_comment != null
                                    && output.prev.order_data.order_data.discharge_free_comment != "")
                                  ) && (
                                  <div className="flex between drug-item table-row">
                                    <div className="text-left">
                                      <div className="table-item">フリーコメント</div>
...
DischargeHistoryModal.contextType = Context;

DischargeHistoryModal.propTypes = {
...
export default DischargeHistoryModal;