...
class AdminDiaryPrintPreview extends Component {
  constructor(props) {
    super(props);
    this.state = {
      numPages: null,
  ...
  async componentDidMount() {
    ...
    axios({
      ...
      data:{
        ...
        staff_table:this.props.staff_table,
  ...
  render() {
    ...
    return  (
      <Modal show={true} onHide={this.onHide} id="add_contact_dlg_print"  className="master-modal medical-info-doc-preview-modal print-sheet-modal">
        <Modal.Header>
          <Modal.Title>看護部管理日誌</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Wrapper>
              ...
              {this.state.file != null ? (
                <>
                  <div>
                    ...
                    <div>
                      <Document className={'flex'} file={this.state.file} onLoadSuccess={this.onDocumentLoadSuccess} renderMode={'svg'}>
                        <Page className={"PDFPage PDFPageOne"} pageNumber={pageNumber} renderTextLayer={false} renderInteractiveForms={false} renderMode={'svg'} width={900} />
                      </Document>
                      <p>ページ {pageNumber} / {numPages}</p>
                    </div>
                  </div>
                  <div className={'loaded-area-file'} id="loaded-area-file" style={{left:file_left,top:file_top}}>
                    <SpinnerWrapper>
...
AdminDiaryPrintPreview.contextType = Context;

AdminDiaryPrintPreview.propTypes = {
  staff_table:PropTypes.array,
  patient_data:PropTypes.array,
  closeModal: PropTypes.func,
  ward_master: PropTypes.array,
};

export default AdminDiaryPrintPreview;
